 ...
 public:
  ...
  MvNorm(const MV& mv,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "(columns) in the MultiVector mv.  normvec.size() = "
            << normvec.size()
    ...
    if (numColsA > numColsMv) {
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          errPrefix << "Input multivector 'A' has " << numColsA << " columns, "
                                                                   "but output multivector 'mv' has only "
                    << numColsMv << " columns.");
