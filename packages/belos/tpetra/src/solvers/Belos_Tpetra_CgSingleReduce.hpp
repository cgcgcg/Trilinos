  ...
  solveOneVec(Teuchos::FancyOStream* outPtr,
    ...
    for (int iter = 0; iter < input.maxNumIters; ++iter) {
      ...
      if (metric <= input.tol) {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(RAR <= ATM::zero(), std::runtime_error, "At iteration " << (iter + 1) << " out of " << input.maxNumIters << ", R.dot(AR) = " << RAR << " <= 0.  This usually means that the matrix A is not symmetric "
