...
struct ComputeRitzValues<LO, SC, false> {
  ...
  run(const int iter,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(info != 0, std::runtime_error,
                               "LAPACK {D,S}SEQR LWORK query failed "
                               "with INFO = "
                                   << info << " != 0.");
    lwork = static_cast<LO>(TEMP);
    TEUCHOS_TEST_FOR_EXCEPTION(lwork < LO{0}, std::runtime_error,
                               "LAPACK {C,Z}SEQR LWORK query "
                               "returned LWORK = "
                                   << lwork << " < 0.");
...
struct ComputeRitzValues<LO, SC, true> {
  ...
  run(const int iter,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(info != 0, std::runtime_error,
                               "LAPACK {C,Z}SEQR LWORK query failed "
                               "with INFO = "
                                   << info << " != 0.");
...
template <class LO, class SC>
void computeRitzValues(const int iter,
                       Teuchos::SerialDenseMatrix<LO, SC>& G,
...
///   Leja ordering.
template <class LO, class SC>
void sortRitzValues(const LO m,
...
class Gmres : public Krylov<SC, MV, OP> {
 ...
 public:
  ...
  setParameters(Teuchos::ParameterList& params) override {
    ...
    bool computeRitzValues = params.get<bool>("Compute Ritz Values", this->input_.computeRitzValues);

    bool needToReortho = params.get<bool>("Reorthogonalize Blocks", this->input_.needToReortho);

    int resCycle = params.get<int>("Num Blocks", this->input_.resCycle);
    TEUCHOS_TEST_FOR_EXCEPTION(resCycle < 0, std::invalid_argument,
                               "\"Num Blocks\" (restart length) = " << resCycle << " < 0.");

    std::string orthoType = params.get<std::string>("Orthogonalization", this->input_.orthoType);

    int maxOrthoSteps = params.get<int>("Max Orthogonalization Passes", this->input_.maxOrthoSteps);
 ...
 protected:
  ...
  setOrthogonalizer(const std::string& ortho) {
    if (ortho_.get() == nullptr || this->input_.orthoType != ortho) {
      TEUCHOS_TEST_FOR_EXCEPTION(this->input_.orthoType == "", std::runtime_error,
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(ortho_.get() == nullptr, std::runtime_error,
                                 "Gmres: Failed to "
                                 "create (Mat)OrthoManager of type \""
                                     << ortho << "\".");
 ...
 private:
  int projectAndNormalize(const int n,
  ...
  int projectAndNormalizeBelosOrthoManager(int n, MV& Q, dense_matrix_type& H,
                                           const SolverInput<SC>& input) {
    using Teuchos::RCP;
    using Teuchos::rcp;
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(ortho_.get() == nullptr, std::logic_error,
                               "Gmres: Failed to create "
 ...
 protected:
  ...
  int normalizeBelosOrthoManager(MV& Q, dense_matrix_type& R) {
    if (ortho_.get() == nullptr) {
...
//#define HAVE_TPETRA_DEBUG
    ...
    while (output.numIters < input.maxNumIters && !output.converged) {
      ...
      for (; iter < restart && metric > input.tol; ++iter) {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(STS::real(H(iter + 1, iter)) < STM::zero(), std::runtime_error,
