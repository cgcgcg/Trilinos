  ...
  solveOneVec(Teuchos::FancyOStream* outPtr,
    ...
    for (int iter = 0; iter < input.maxNumIters; ++iter) {
      ...
      if (iter == 0) {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(ATS::real(RAR) <= ATM::zero(), std::runtime_error,
                                   "At iteration " << (iter + 1)
                                                   << " out of " << input.maxNumIters << ", R.dot(AR) = " << RAR << " <= 0.  This usually means that the matrix A is not symmetric "
