...
class GmresSstep : public Gmres<SC, MV, OP> {
 ...
 public:
  ...
  setParameters(Teuchos::ParameterList& params) {
    ...
    bool computeRitzValuesOnFly         = params.get<bool>("Compute Ritz Values on Fly", this->input_.computeRitzValuesOnFly);
      ...
      for (; iter < restart && metric > input.tol; iter += step) {
        ...
        if (rank == step + 1 && H(iter + step, iter + step - 1) != zero) {
          ...
          for (int iiter = 0; iiter < step; iiter++) {
            // Check negative norm
            TEUCHOS_TEST_FOR_EXCEPTION(STS::real(H(iter + iiter + 1, iter + iiter)) < STM::zero(),
                                       std::runtime_error, "At iteration " << output.numIters << ", H(" << iter + iiter + 1 << ", " << iter + iiter << ") = " << H(iter + iiter + 1, iter + iiter) << " < 0.");
        ...
        if (input.computeRitzValuesOnFly && int(output.ritzValues.size()) == 0 && output.numIters >= input.stepSize) {
 ...
 protected:
  ...
  int normalizeCholQR(Teuchos::FancyOStream* outPtr,
                      const int n,
                      const int s,
                      MV& Q,
                      dense_matrix_type& R) {
  ...
  //! Apply the orthogonalization using Belos' OrthoManager
  int recursiveCholQR(Teuchos::FancyOStream* outPtr,
                      const int n,
                      const int s,
                      MV& Q,
                      dense_matrix_type& R) {
