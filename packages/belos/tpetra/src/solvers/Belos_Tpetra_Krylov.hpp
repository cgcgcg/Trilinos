...
class Krylov {
 ...
 public:
  ...
  getParameters(Teuchos::ParameterList& params,
    ...
    if (defaultValues) {
      ...
      const std::string implResScal = input_.needToScale ? "Norm of Preconditioned Initial Residual" : "None";  // ???
  ...
  getDefaultParameters(Teuchos::ParameterList& params) const {
    ...
    const std::string implResScal = input.needToScale ? "Norm of Preconditioned Initial Residual" : "None";  // ???
 ...
 protected:
  ...
  getConvergenceMetric(const mag_type r_norm_new,
                       ...
                       const SolverInput<SC>& input) {
    if (input.needToScale) {
      return r_norm_orig == STM::zero() ? r_norm_new : (r_norm_new / r_norm_orig);
    } else {
      return r_norm_new;
    }
  }
 ...
 private:
  ...
  solveImpl(Teuchos::FancyOStream* outPtr, MV& X, const MV& B) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(M_.get() == nullptr && input_.precoSide != "none",
                               std::logic_error, prefix << "The preconditioner M_ is null, but "
                                                           "input_.precoSide=\""
                                                        << input_.precoSide << "\" != \"none\".  "
                                                                               "Please report this bug to the Belos developers.");

    const size_t numVecs = B.getNumVectors();
    TEUCHOS_TEST_FOR_EXCEPTION(X.getNumVectors() != numVecs, std::runtime_error, prefix << "X.getNumVectors() = " << X.getNumVectors() << " != B.getNumVectors() = " << numVecs << ".");
