...
class GmresSingleReduce : public Gmres<SC, MV, OP> {
 ...
 private:
  ...
  int projectAndNormalizeSingleReduce_cleanup(Teuchos::FancyOStream* outPtr,
                                              int n,
  ...
  int projectAndNormalizeSingleReduce(Teuchos::FancyOStream* outPtr,
                                      int n,
                                      const SolverInput<SC>& input,
                                      MV& Q,
                                      dense_matrix_type& H,
  ...
  int projectAndNormalizeSingleReduce_GS(Teuchos::FancyOStream* outPtr,
                                         int n,
                                         const SolverInput<SC>& input,
                                         MV& Q,
                                         dense_matrix_type& H,
    ...
    Teuchos::RCP<dense_matrix_type> tj = Teuchos::rcp(new dense_matrix_type(Teuchos::View, T, n + 2, 2, 0, n));
    {
      Teuchos::TimeMonitor LocalTimer(*dotsTimer);
    ...
    if (input.delayedNorm) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(prevNorm < STM::zero(), std::runtime_error, "At iteration " << n << ", T(" << n << ", " << n << ") = " << T(n, n) << " < 0.");
    ...
    if (newNorm <= oldNorm * tolOrtho) {
      ...
      // check
      TEUCHOS_TEST_FOR_EXCEPTION(newNorm < STM::zero(), std::runtime_error, "At iteration " << n << ", H(" << n + 1 << ", " << n << ") = " << newNorm << " < 0.");
  ...
  //! CGS1 specialization
  int projectAndNormalizeSingleReduce_CGS1(Teuchos::FancyOStream* outPtr,
                                           int n,
                                           const SolverInput<SC>& input,
                                           MV& Q,
                                           dense_matrix_type& H,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(STS::real(H(n + 1, n)) < STM::zero(), std::runtime_error, "At iteration " << n << ", H(" << n + 1 << ", " << n << ") = " << H(n + 1, n) << " < 0.");
