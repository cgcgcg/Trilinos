...
void testSolver(Teuchos::FancyOStream& out,
  ...
  out << "Apply solver to \"solve\" AX=B for X.  Belos already has solver tests;"
         " the point is to check that solve() doesn't throw if it encounters a NaN."
      << endl;
...
template <class SC, class LO, class GO, class NT>
void testCreatingSolver(Teuchos::FancyOStream& out,
                        bool& success,
                        const std::string& solverName) {
  using std::endl;
  using Teuchos::RCP;
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(SolverFactory, CreateSolvers, SC, LO, GO, NT) {
  ...
  Teuchos::OSTab tab1(out);
  const std::vector<std::string> solverNames{{"BICGSTAB",
                                              "BLOCK CG",
                                              "BLOCK GMRES",
                                              "TPETRA CG PIPELINE",
