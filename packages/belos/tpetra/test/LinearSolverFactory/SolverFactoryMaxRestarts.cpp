...
void createTestProblem(Teuchos::FancyOStream& out,
  ...
  for (size_t j = 0; j < numVecs; ++j) {
    // Only modify if this processor owns local row 0
    if (B->getMap()->isNodeLocalElement(0)) {
      B->replaceLocalValue(0, j, STS::one());
    }
  }
...
void testSolver(Teuchos::FancyOStream& out,
  ...
  out << "Apply solver to \"solve\" AX=B for X, and check if it fails to converge with 'MaxRestartsReached'." << endl;
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(SolverFactory, CreateAndSolve, SC, LO, GO, NT) {
  ...
  Belos::SolverFactory<SC, MV, OP> factory;
  const char* solverNames[3] = {
      "BLOCK GMRES",
      "GCRODR",
      "PSEUDOBLOCK GMRES"};
  const int numSolvers = 3;
