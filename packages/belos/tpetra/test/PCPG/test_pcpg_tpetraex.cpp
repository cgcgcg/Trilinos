...
int run(Teuchos::CommandLineProcessor& cmdp, int argc, char* argv[]) {
  ...
  try {
    ...
    RCP<Belos::LinearProblem<ST, MV, OP, DM> > problem = rcp(new Belos::LinearProblem<ST, MV, OP, DM>(A, LHS, RHS));
    ...
    RCP<Belos::SolverManager<ST, MV, OP, DM> > solver = rcp(new Belos::PCPGSolMgr<ST, MV, OP, DM>(problem, rcp(&belosList, false)));
