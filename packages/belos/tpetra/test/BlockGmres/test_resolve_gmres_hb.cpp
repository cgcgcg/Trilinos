...
int run(Teuchos::CommandLineProcessor& cmdp, int argc, char* argv[]) {
  ...
  try {
    ...
    MVT::MvNorm(*B, rhs_norm);
    if (procVerbose) {
      std::cout << "---------- Actual Residuals (manager reset) ----------" << std::endl
                << std::endl;
      for (int i = 0; i < numrhs; i++) {
        std::cout << "Problem " << i << " : \t" << actual_resids[i] / rhs_norm[i] << std::endl;
    ...
    MVT::MvNorm(*B, rhs_norm);
    if (procVerbose) {
      std::cout << "---------- Actual Residuals (manager setProblem()) ----------" << std::endl
                << std::endl;
      for (int i = 0; i < numrhs; i++) {
        std::cout << "Problem " << i << " : \t" << actual_resids[i] / rhs_norm[i] << std::endl;
        if (actual_resids2[i] > SCT::prec()) {
    ...
    MVT::MvNorm(*B, rhs_norm);
    if (procVerbose) {
      std::cout << "---------- Actual Residuals (label reset) ----------" << std::endl
                << std::endl;
      for (int i = 0; i < numrhs; i++) {
        std::cout << "Problem " << i << " : \t" << actual_resids[i] / rhs_norm[i] << std::endl;
        if (actual_resids2[i] > SCT::prec()) {
    ...
    if (procVerbose) {
      if (pseudo)
        std::cout << std::endl
                  << "Valid parameters from the pseudo-block Gmres solver manager:" << std::endl;
      else
        std::cout << std::endl
                  << "Valid parameters from the block Gmres solver manager:" << std::endl;
    ...
    MVT::MvNorm(*B, rhs_norm);
    if (procVerbose) {
      std::cout << "---------- Actual Residuals (new solver) ----------" << std::endl
                << std::endl;
      for (int i = 0; i < numrhs; i++) {
        std::cout << "Problem " << i << " : \t" << actual_resids[i] / rhs_norm[i] << std::endl;
        if (actual_resids2[i] > SCT::prec()) {
