...
int run(Teuchos::CommandLineProcessor& cmdp, int argc, char* argv[]) {
  ...
  try {
    ...
    if (set == false) {
      if (procVerbose)
        std::cout << std::endl
                  << "ERROR:  Initial Belos::LinearProblem failed to set up correctly!" << std::endl;
      return -1;
    }
    ...
    RCP<Belos::SolverManager<ST, MV, OP, DM> > initSolver = rcp(new Belos::PseudoBlockGmresSolMgr<ST, MV, OP, DM>(rcp(&initProblem, false), rcp(&belosList, false)));
    ...
    if (set == false) {
      if (procVerbose)
        std::cout << std::endl
                  << "ERROR:  Augmented Belos::LinearProblem failed to set up correctly!" << std::endl;
      return -1;
    }
    ...
    RCP<Belos::SolverManager<ST, MV, OP, DM> > augSolver = rcp(new Belos::PseudoBlockGmresSolMgr<ST, MV, OP, DM>(rcp(&augProblem, false), rcp(&belosList, false)));
