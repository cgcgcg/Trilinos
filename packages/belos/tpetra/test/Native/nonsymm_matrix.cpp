...
TEUCHOS_STATIC_SETUP() {
  ...
  clp.setOption("zeroInitialGuess", "nonzeroInitialGuess",
                &commandLineOptions.zeroInitialGuess,
                "Whether to test "
...
createNonsymmTridiagMatrix(const Teuchos::RCP<const Tpetra::Map<> >& rowMap,
  ...
  const LO lclNumRows   = rowMap.is_null() ? LO(0) : LO(rowMap->getLocalNumElements());
...
void testSolver(Teuchos::FancyOStream& out,
  ...
  const mag_type tol = [&]() {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(!pl->isType<mag_type>(tolParamName), std::logic_error,
  ...
  for (size_t j = 0; j < R_final.getNumVectors(); ++j) {
    const mag_type relResNorm = (R_initial_norms[j] == STM::zero()) ? R_final_norms[j] : R_final_norms[j] / R_initial_norms[j];
