...
int run(Teuchos::CommandLineProcessor& cmdp, int argc, char* argv[]) {
  ...
  try {
    ...
    cmdp.setOption("frequency", &frequency,
                   "Solver's frequency for printing "
    ...
    cmdp.setOption("filename", &filename,
                   "Filename for Harwell-Boeing test "
                   "matrix.");
    cmdp.setOption("num-rhs", &numrhs,
                   "Number of right-hand sides for which "
                   "to solve.");
