...
int run(int argc, char *argv[]) {
  ...
  try {
    ...
    if (set == false) {
      if (procVerbose)
        std::cout << std::endl
                  << "ERROR:  Initial Belos::LinearMultiShiftProblem failed to set up correctly!" << std::endl;
      return -1;
    }
    ...
    RCP<Belos::SolverManager<ST, MV, OP> > initSolver = rcp(new Belos::PseudoBlockGmresSolMgr<ST, MV, OP>(rcp(&initProblem, false), rcp(&belosList, false)));
    ...
    if (ret == Belos::Converged && !badRes) {
      ...
      if (procVerbose)
        std::cout << std::endl
                  << "ERROR:  Initial solve did not converge to solution!" << std::endl;
      return -1;
    }
    ...
    if (set == false) {
      if (procVerbose)
        std::cout << std::endl
                  << "ERROR:  Augmented Belos::LinearMultiShiftProblem failed to set up correctly!" << std::endl;
      return -1;
    }
    ...
    RCP<Belos::SolverManager<ST, MV, OP> > augSolver = rcp(new Belos::PseudoBlockGmresSolMgr<ST, MV, OP>(rcp(&augProblem, false), rcp(&belosList, false)));
    ...
    if (ret == Belos::Converged) {
      ...
      if (procVerbose)
        std::cout << std::endl
                  << "ERROR: Augmented solver did not converge to solution!" << std::endl;
      return -1;
    }
