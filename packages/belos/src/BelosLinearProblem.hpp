...
class LinearProblem {
 public:
  ...
  virtual Teuchos::RCP<MV>
  updateSolution(const Teuchos::RCP<MV>& update = Teuchos::null,
                 ScalarType scale               = Teuchos::ScalarTraits<ScalarType>::one()) const { return const_cast<LinearProblem<ScalarType, MV, OP, DM>*>(this)->updateSolution(update, false, scale); }
...
LinearProblem<ScalarType, MV, OP, DM>::
  ...
  RCP<MV> newSoln;
  if (update.is_null()) {  // The caller didn't supply an update vector, so we assume
  ...
  } else  // the update vector is NOT null.
  {
    ...
    {
      if (RP_.is_null()) {  // There is no right preconditioner.
        // curX_ := curX_ + scale * update.
        MVT::MvAddMv(1.0, *curX_, scale, *update, *curX_);
      } else {  // There is indeed a right preconditioner, so apply it
      ...
      newSoln          = curX_;
    } else {  // Rather than updating our stored current solution curX_,
...
template <class ScalarType, class MV, class OP, class DM>
bool LinearProblem<ScalarType, MV, OP, DM>::
  ...
  if (Teuchos::is_null(R0_user_)) {
    ...
    if (LP_ != Teuchos::null) {
      ...
      if (PR0_user_ == Teuchos::null || (PR0_user_ == R0_) || (PR0_user_ == R0_user_) || (MVT::GetNumberVecs(*PR0_user_) != MVT::GetNumberVecs(*B_))) {
        Teuchos::RCP<MV> helper = MVT::Clone(*B_, MVT::GetNumberVecs(*B_));
