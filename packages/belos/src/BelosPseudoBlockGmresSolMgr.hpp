 ...
 *
 */
class PseudoBlockGmresSolMgrLinearProblemFailure : public BelosError {
 public:
 ...
 *
 */
class PseudoBlockGmresSolMgrOrthoFailure : public BelosError {
 public:
 ...
 public:
  ...
  virtual void setUserConvStatusTest(
      ...
      const typename StatusTestCombo<ScalarType, MV, OP, DM>::ComboType &comboType =
          StatusTestCombo<ScalarType, MV, OP, DM>::SEQ) override;
...
void PseudoBlockGmresSolMgr<ScalarType, MV, OP, DM>::
  ...
  if (params->isParameter("Deflation Quorum")) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "Belos::PseudoBlockGmresSolMgr::setParameters: "
        "The \"Deflation Quorum\" parameter (= "
            << defQuorum_ << ") must not be "
...
ReturnType PseudoBlockGmresSolMgr<ScalarType, MV, OP, DM>::solve() {
  ...
  Teuchos::RCP<PseudoBlockGmresIter<ScalarType, MV, OP, DM>> block_gmres_iter = Teuchos::rcp(new PseudoBlockGmresIter<ScalarType, MV, OP, DM>(problem_, printer_, outputTest_, ortho_, plist));
        ...
        try {
          ...
          if (convTest_->getStatus() == Passed) {
            if (expConvTest_->getLOADetected()) {
              ...
              loaDetected_ = true;
              printer_->stream(Warnings) << "Belos::PseudoBlockGmresSolMgr::solve(): Warning! Solver has experienced a loss of accuracy!" << std::endl;
              isConverged = false;
            }
