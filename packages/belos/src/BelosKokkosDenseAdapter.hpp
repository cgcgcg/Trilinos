...
void kokkos_transpose(const V& dst, const V& src) {
  Kokkos::parallel_for(
      Kokkos::MDRangePolicy<typename V::execution_space, Kokkos::Rank<2>>({0, 0}, {dst.extent(0), dst.extent(1)}),
  ...
  */
  ...
  CreateCopy(const DM& dm, bool transpose = false) {
    ...
    if (transpose) {
      ...
      tmpCopyRCP = Teuchos::rcp(new DM(Kokkos::view_alloc(Kokkos::WithoutInitializing, "BelosDenseCreateCopy"), dm.extent_int(1), dm.extent_int(0)));
      ...
      tmpCopyRCP = Teuchos::rcp(new DM(Kokkos::view_alloc(Kokkos::WithoutInitializing, "BelosDenseCreateCopy"), dm.extent_int(0), dm.extent_int(1)));
  ...
  SubviewCopy(const DM& source, int numRows, int numCols, int startRow = 0, int startCol = 0) {
    ...
    auto tmpViewRCP = Teuchos::rcp(new DM(Kokkos::view_alloc(Kokkos::WithoutInitializing, "BelosDenseSubViewCopy"), numRows, numCols));
  ...
  static typename Teuchos::ScalarTraits<Scalar>::magnitudeType NormFrobenius(const DM& dm) {
    ...
    Kokkos::parallel_reduce(
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {dm.extent(0), dm.extent(1)}),
  ...
  static typename Teuchos::ScalarTraits<Scalar>::magnitudeType NormOne(const DM& dm) {
    ...
    Kokkos::parallel_reduce(
        dm.extent(1), KOKKOS_LAMBDA(const int j, mag_t& norm) {
          mag_t sum = 0;
  ...
  createDenseSolver() {
    Teuchos::RCP<DenseSolver<Scalar, DM>> newSolver = Teuchos::rcp(new KokkosDenseSolver<Scalar, DM>());
    return newSolver;
  }
  //@}
};
