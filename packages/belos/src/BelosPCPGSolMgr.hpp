 ...
 *
 */
class PCPGSolMgrOrthoFailure : public BelosError {
 public:
 ...
 *
 */
class PCPGSolMgrLAPACKFailure : public BelosError {
 public:
...
class PCPGSolMgr : public Details::SolverManagerRequiresRealLapack<ScalarType, MV, OP, DM,
                                                                   Belos::Details::LapackSupportsScalar<ScalarType>::value &&
                                                                       !Teuchos::ScalarTraits<ScalarType>::isComplex> {
...
class PCPGSolMgr<ScalarType, MV, OP, DM, true> : public Details::SolverManagerRequiresRealLapack<ScalarType, MV, OP, DM, true> {
 private:
...
ReturnType PCPGSolMgr<ScalarType, MV, OP, DM, true>::solve() {
  ...
  // Enter solve() iterations
  {
    ...
    while (numRHS2Solve > 0) {  // test for quick return
      ...
      if (!dimU_) printer_->stream(Debug) << " No recycled subspace available for RHS index " << currIdx[0] << std::endl
                                          << std::endl;
      ...
      printer_->stream(Debug) << " Generated recycled subspace using RHS index " << currIdx[0] << " of dimension " << dimU_ << std::endl
                              << std::endl;
