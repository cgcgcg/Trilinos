...
template <class ScalarType, class MV, class OP, class DM>
void BlockGCRODRSolMgr<ScalarType, MV, OP, DM>::
  ...
  //
  {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(numBlocks <= 0, std::invalid_argument,
                               ...
                               "positive, but you specified a nonpositive value of "
                                   << numBlocks
                                   << ".");
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(recycledBlocks >= numBlocks,
                               ...
                               "but you specified \"Num Recycled Blocks\" = "
                                   << recycledBlocks
...
template <class ScalarType, class MV, class OP, class DM>
void BlockGCRODRSolMgr<ScalarType, MV, OP, DM>::initializeStateStorage() {
...
void BlockGCRODRSolMgr<ScalarType, MV, OP, DM>::buildRecycleSpaceKryl(int& keff, Teuchos::RCP<BlockGmresIter<ScalarType, MV, OP, DM>> block_gmres_iter) {
  ...
  if (recycledBlocks_ >= p + blockSize_) {  // keep whole block Krylov subspace
    ...
    for (int ii = 0; ii < keff; ii++) {
      for (int jj = ii; jj < keff; jj++) Rtmp(ii, jj) = HPtmp(ii, jj);
    }
    ...
    for (int ii = 0; ii < (p + blockSize_); ++ii) {
      index[ii] = ii;
    }
...
void BlockGCRODRSolMgr<ScalarType, MV, OP, DM>::buildRecycleSpaceAugKryl(Teuchos::RCP<BlockGCRODRIter<ScalarType, MV, OP>> block_gcrodr_iter) {
  ...
  if (recycledBlocks_ >= keff + p) {  // we add new Krylov vectors to existing recycle space
    ...
    for (int ii = 0; ii < p; ++ii) {
      index[ii] = keff + ii;
    }  // get a view after current reycle vectors
    Teuchos::RCP<MV> Utmp = MVT::CloneViewNonConst(*U_, index);
    for (int ii = 0; ii < p; ++ii) {
      index[ii] = ii;
    }
  ...
  for (int i = 0; i < keff_new; i++) {
    for (int j = i; j < keff_new; j++) Ftmp(i, j) = HPtmp(i, j);
  }
  // lapack.ORGQR(HPtmp.numRows(),HPtmp.numCols(),HPtmp.numCols(),HPtmp.values(),HPtmp.stride(),&tau_[0],&work_[0],lwork,&info);
  lapack.UNGQR(HPtmp.numRows(), HPtmp.numCols(), HPtmp.numCols(), HPtmp.values(), HPtmp.stride(), &tau_[0], &work_[0], lwork, &info);
  TEUCHOS_TEST_FOR_EXCEPTION(info != 0, BlockGCRODRSolMgrLAPACKFailure, "Belos::BlockGCRODRSolMgr::solve(): LAPACK _UNGQR failed to construct the Q factor.");
...
ReturnType BlockGCRODRSolMgr<ScalarType, MV, OP, DM>::solve() {
  ...
  while (numRHS2Solve > 0) {  // This loops through each block of RHS's being solved
    ...
    if (keff > 0) {  // If there is already a subspace to recycle, then use it
      ...
      printer_->stream(Debug) << " Now solving RHS index " << currIdx[0] << " using recycled subspace of dimension " << keff << std::endl
                              << std::endl;
      ...
      printer_->stream(Debug) << " No recycled subspace available for RHS index " << std::endl
                              << std::endl;
      ...
      try {
        printer_->stream(Debug) << " Preparing to Iterate!!!!" << std::endl
                                << std::endl;
        ...
        if (convTest_->getStatus() == Passed) {
          printer_->stream(Debug) << "We converged during the prime the pump stage" << std::endl
                                  << std::endl;
        ...
        // ****************************************************************
          ...
          printer_->stream(Debug) << " We did not converge on priming cycle of Block GMRES.  Therefore we recycle and restart. " << std::endl
                                  << std::endl;
        }
      }  // end try
      ...
      printer_->stream(Debug) << "Generated recycled subspace using RHS index " << currIdx[0] << " of dimension " << keff << std::endl
                              << std::endl;
    ...
    while (1) {  // Each execution of this loop is a cycle of block GCRODR
      try {
        ...
        // **********************************************
          ...
          printer_->stream(Debug) << " Generated new recycled subspace using RHS index " << currIdx[0] << " of dimension " << keff << std::endl
                                  << std::endl;
          ...
          printer_->stream(Debug) << " Performing restart number " << numRestarts << " of " << maxRestarts_ << std::endl
                                  << std::endl;
