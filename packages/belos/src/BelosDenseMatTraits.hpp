...
class DenseMatTraits {
  ...
  */
   ...
   */
  static void Reshape(DM& dm, const int numrows, const int numcols, bool initZero = false) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }
  ...
  static ScalarType& Value(DM& dm, const int i, const int j) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //! \brief Access a const reference to the (i,j) entry of \c dm, \c e_i^T dm e_j.
  static const ScalarType& ValueConst(const DM& dm, const int i, const int j) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  static void SyncDeviceToHost(DM& dm) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  static void SyncHostToDevice(DM& dm) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //@}
  //@{ \name Operator methods

  //!  \brief Adds sourceDM to thisDM and returns answer in thisDM.
  static void Add(DM& thisDM, const DM& sourceDM) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //!  \brief Fill all entries with \c value. Value is zero if not specified.
  static void PutScalar(DM& dm, ScalarType value = Teuchos::ScalarTraits<ScalarType>::zero()) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //!  \brief Multiply all entries by a scalar. DM = value.*DM
  static void Scale(DM& dm, ScalarType value) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //!  \brief Fill the DM with random entries.
  //!   Entries are assumed to be the same on each MPI rank (each matrix copy).
  static void Randomize(DM& dm) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //!  \brief Copies entries of sourceDM to thisDM (deep copy).
  static void Assign(DM& thisDM, const DM& sourceDM) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //!  \brief Returns the Frobenius norm of the dense matrix.
  static typename Teuchos::ScalarTraits<ScalarType>::magnitudeType NormFrobenius(const DM& dm) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }

  //!  \brief Returns the one-norm of the dense matrix.
  static typename Teuchos::ScalarTraits<ScalarType>::magnitudeType NormOne(const DM& dm) { UndefinedDenseMatTraits<ScalarType, DM>::notDefined(); }
