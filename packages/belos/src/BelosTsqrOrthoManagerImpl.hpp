...
class TsqrOrthoManagerImpl : public Teuchos::ParameterListAcceptorDefaultBase {
 public:
  typedef Scalar scalar_type;
  typedef typename Teuchos::ScalarTraits<Scalar>::magnitudeType magnitude_type;
 ...
 private:
  ...
  ///
  /// \return Rank of X_in after projection
  int projectAndNormalizeImpl(MV& X_in,
                              MV& X_out,
...
template <class Scalar, class MV, class DM>
void TsqrOrthoManagerImpl<Scalar, MV, DM>::
...
template <class Scalar, class MV, class DM>
void TsqrOrthoManagerImpl<Scalar, MV, DM>::project(MV& X,
                                                   Teuchos::Array<mat_ptr> C,
                                                   Teuchos::ArrayView<Teuchos::RCP<const MV> > Q) {
#ifdef BELOS_TEUCHOS_TIME_MONITOR
...
template <class Scalar, class MV, class DM>
int TsqrOrthoManagerImpl<Scalar, MV, DM>::normalize(MV& X, mat_ptr B) {
...
template <class Scalar, class MV, class DM>
int TsqrOrthoManagerImpl<Scalar, MV, DM>::
...
int TsqrOrthoManagerImpl<Scalar, MV, DM>::
  ...
  const int firstPassRank = outOfPlace ? normalizeOutOfPlace(X_in, X_out, B_out) : normalize(X_in, B_out);
  if (B.is_null()) {
  ...
  if (reorthogonalizeBlocks_) {
    ...
    if (reorthogonalize) {
      ...
      const int secondPassRank = outOfPlace ? normalizeOutOfPlace(X_in, X_out, B2) : normalize(X_in, B2);
    ...
    // purposes.
    {
      ...
      if (anyZero) {
        ...
        os << "TsqrOrthoManagerImpl::normalizeImpl: "
              ...
              "with "
           << numCols << " columns and reported column rank "
           << rank << ".  The inclusive range of columns to fill with "
                      "random data is ["
           << nullSpaceIndices.lbound() << ","
