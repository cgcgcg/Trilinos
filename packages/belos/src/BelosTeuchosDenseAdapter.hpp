...
class DenseMatTraits<ScalarType, Teuchos::SerialDenseMatrix<int, ScalarType>> {
  ...
  */
  ...
  static ScalarType* GetRawHostPtr(Teuchos::SerialDenseMatrix<int, ScalarType>& dm) { return dm.values(); }

  //! \brief Returns a raw pointer to const data on the host.
  static ScalarType const* GetConstRawHostPtr(const Teuchos::SerialDenseMatrix<int, ScalarType>& dm) { return const_cast<ScalarType const*>(dm.values()); }
  ...
  static Teuchos::RCP<Teuchos::SerialDenseMatrix<int, ScalarType>> Subview(Teuchos::SerialDenseMatrix<int, ScalarType>& source, int numRows, int numCols, int startRow = 0, int startCol = 0) { return Teuchos::rcp(new Teuchos::SerialDenseMatrix<int, ScalarType>(Teuchos::View, source, numRows, numCols, startRow, startCol)); }

  static Teuchos::RCP<const Teuchos::SerialDenseMatrix<int, ScalarType>> SubviewConst(
      const Teuchos::SerialDenseMatrix<int, ScalarType>& source, int numRows, int numCols, int startRow = 0, int startCol = 0) { return Teuchos::rcp(new const Teuchos::SerialDenseMatrix<int, ScalarType>(Teuchos::View, source, numRows, numCols, startRow, startCol)); }

  //! \brief Returns a deep copy of the requested subview.
  static Teuchos::RCP<Teuchos::SerialDenseMatrix<int, ScalarType>> SubviewCopy(const Teuchos::SerialDenseMatrix<int, ScalarType>& source, int numRows, int numCols, int startRow = 0, int startCol = 0) { return Teuchos::rcp(new Teuchos::SerialDenseMatrix<int, ScalarType>(Teuchos::Copy, source, numRows, numCols, startRow, startCol)); }
  //@}

  //@{ \name Attribute methods

  //! \brief Obtain the number of rows of \c dm.
  static int GetNumRows(const Teuchos::SerialDenseMatrix<int, ScalarType>& dm) { return dm.numRows(); }

  //! \brief Obtain the number of columns of \c dm.
  static int GetNumCols(const Teuchos::SerialDenseMatrix<int, ScalarType>& dm) { return dm.numCols(); }

  //! \brief Obtain the stride between the columns of \c dm.
  static int GetStride(const Teuchos::SerialDenseMatrix<int, ScalarType>& dm) { return dm.stride(); }
  ...
  static void Reshape(Teuchos::SerialDenseMatrix<int, ScalarType>& dm, const int numrows, const int numcols, bool initZero = false) {
    if (initZero) {
      ...
      if (err != 0) {
        throw std::runtime_error("Error in DenseMatrixTraits::shape. Teuchos::SerialDenseMatrix.shape failed.");
      }
    } else {
      int err = dm.reshape(numrows, numcols);
      if (err != 0) {
        throw std::runtime_error("Error in DenseMatrixTraits::reshape. Teuchos::SerialDenseMatrix.reshape failed.");
      }
    }
  }
  ...
  createDenseSolver() {
    Teuchos::RCP<DenseSolver<ScalarType, Teuchos::SerialDenseMatrix<int, ScalarType>>> newSolver = Teuchos::rcp(new TeuchosDenseSolver<ScalarType>());
    return newSolver;
  }
  //@}
};
