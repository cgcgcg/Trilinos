...
class GCRODRSolMgr : public Details::SolverManagerRequiresLapack<ScalarType, MV, OP, DM> {
...
class GCRODRSolMgr<ScalarType, MV, OP, DM, true> : public Details::SolverManagerRequiresLapack<ScalarType, MV, OP, DM, true> {
#if defined(HAVE_TEUCHOSCORE_CXX11)
#if defined(HAVE_TEUCHOS_COMPLEX)
...
ReturnType GCRODRSolMgr<ScalarType, MV, OP, DM, true>::solve() {
  ...
  if (static_cast<ptrdiff_t>(numBlocks_) > dim) {
    ...
    printer_->stream(Warnings) << "Warning! Requested Krylov subspace dimension is larger than operator dimension!" << std::endl
  ...
  // Enter solve() iterations
  {
    ...
    while (numRHS2Solve > 0) {
      ...
      if (keff > 0) {
        ...
        printer_->stream(Debug) << " No recycled subspace available for RHS index " << currIdx[0] << std::endl
                                << std::endl;
        ...
        for (int ii = 0; ii < (numBlocks_ + 1); ++ii) {
          index[ii] = ii;
        }
...
void GCRODRSolMgr<ScalarType, MV, OP, DM, true>::buildRecycleSpace2(Teuchos::RCP<GCRODRIter<ScalarType, MV, OP, DM> > gcrodr_iter) {
  ...
  for (int i = 0; i < keff_new; i++) {
    for (int j = i; j < keff_new; j++) DMT::Value(*Rtmp, i, j) = DMT::ValueConst(*HPtmp, i, j);
  }
