 ...
 public:
  ...
  matAdd(mat_type& A, const mat_type& B) const {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "A is "
            << numRows << " x " << numCols << ", but B is " << B.numRows() << " x " << B.numCols() << ".");
    if (numRows == 0 || numCols == 0) {
      return;
    } else {
      for (int j = 0; j < numCols; ++j) {
  ...
  ensureUpperHessenberg(const mat_type& A,
    ...
    if (result.first) {
      ...
      const magnitude_type err = (result.second.second == STM::zero() ? result.second.first : result.second.first / result.second.second);
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(A.numRows() < minNumRows || A.numCols() < minNumCols,
                               ...
                               "The matrix " << matrixName << " is " << A.numRows()
                                             << " x " << A.numCols() << ", and therefore does not "
                                                                        "satisfy the minimum dimensions "
                                             << minNumRows
                                             << " x " << minNumCols << ".");
  }
...
ERobustness inline robustnessStringToEnum(const std::string& x) {
  const char* strings[] = {"None", "Some", "Lots"};
...
class ProjectedLeastSquaresSolver {
 ...
 public:
  ...
  testUpdateColumn(std::ostream& out,
    ...
    const magnitude_type blockGivensSolutionError = testBlockGivens ? solutionError(y_blockGivens_view, y_lapack_view) : STM::zero();
 ...
 private:
  ...
  leastSquaresConditionNumber(const mat_type& A,
    ...
    // at 1.
    const magnitude_type cosTheta = (sinTheta > STM::one()) ? STM::zero() : STM::squareroot(1 - sinTheta * sinTheta);
