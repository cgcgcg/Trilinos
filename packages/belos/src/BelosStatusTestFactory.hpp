...
class StatusTestFactory {
 public:
  ...
  static typename StatusTestCombo<Scalar, MV, OP, DM>::ComboType stringToComboType(const std::string& comboString) {
    ...
    comboType userCombo;
    if (comboString == "AND")
      userCombo = combo_test::AND;
    else if (comboString == "OR")
      userCombo = combo_test::OR;
    else if (comboString == "SEQ")
      userCombo = combo_test::SEQ;
    else
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "StatusTestFactory:stringToComboType: The \"Combo Type\" must be \"AND\", \"OR\" or \"SEQ\".");
    return userCombo;
  }
 ...
 private:
  ...
  Teuchos::RCP<base_test> buildResidualNormTest(Teuchos::ParameterList& p) const {
    ...
    if (residual_type_string == "Explicit")
      residual_type = res_norm_test::Explicit;
    else if (residual_type_string == "Implicit")
      residual_type = res_norm_test::Implicit;
    else
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "Belos::StatusTestFactory::buildResidualNormTest: The \"Residual Type\" must be \"Explicit\" or \"Implicit\".");
