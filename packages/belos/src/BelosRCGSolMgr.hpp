 ...
 *
 */
class RCGSolMgrLinearProblemFailure : public BelosError {
 public:
 ...
 *
 */
class RCGSolMgrLAPACKFailure : public BelosError {
 public:
...
class RCGSolMgr : public Details::SolverManagerRequiresRealLapack<ScalarType, MV, OP, DM,
                                                                  Belos::Details::LapackSupportsScalar<ScalarType>::value &&
                                                                      !Teuchos::ScalarTraits<ScalarType>::isComplex> {
...
class RCGSolMgr<ScalarType, MV, OP, DM, true> : public Details::SolverManagerRequiresRealLapack<ScalarType, MV, OP, DM, true> {
 private:
  ...
  // Enter solve() iterations
  {
    ...
    while (numRHS2Solve > 0) {
      ...
      if (printer_->isVerbosity(Debug)) {
        if (existU_)
          printer_->print(Debug, "Using recycle space generated from previous call to solve().");
        else
          printer_->print(Debug, "No recycle space exists.");
      }
