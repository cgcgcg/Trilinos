...
bool TestMultiVecTraits(const Teuchos::RCP<OutputManager<ScalarType>> &om,
    ...
    for (int i = 0; i < numvecs; i++) {
      if (norms[i] < zero_mag) {
        ...
        om->stream(Warnings)
            ...
            << "norms[i]: " << norms[i] << "\tdim: " << MVT::GetGlobalLength(*B) << endl
            << endl;
        BadNormWarning = true;
      }
    }
  ...
  *********************************************************************/
  {
    ...
    for (int i = 0; i < p; i++) {
      for (int j = 0; j < q; j++) {
        if (STS::magnitude(DMT::ValueConst(*SDM, i, j)) > STS::magnitude(normsB[i] * normsC[j])) {
          om->stream(Warnings)
              << "*** ERROR *** MultiVecTraits::MvTransMv()." << endl
  ...
  *********************************************************************/
  {
    ...
    for (int i = 0; i < BELOS_MIN(p, q); i++) {
      if (STS::magnitude(iprods[i]) > STS::magnitude(normsB[i] * normsC[i])) {
        om->stream(Warnings)
            << "*** ERROR *** MultiVecTraits::MvDot()." << endl
...
bool TestOperatorTraits(const Teuchos::RCP<OutputManager<ScalarType>> &om,
                        const Teuchos::RCP<const MV> &A,
  ...
  for (int i = 0; i < numvecs; i++) {
    ...
    if (normsC1[i] != normsC2[i] && !NonDeterministicWarning) {
      om->stream(Warnings)
          ...
          << "Apply() returned two different results." << endl
          << endl;
