...
bool TestKokkosMultiVecTraits(const Teuchos::RCP<OutputManager<ScalarType> > &om,
  ...
  *********************************************************************/
  {
    ...
    for (int i = 0; i < p; i++) {
      for (int j = 0; j < q; j++) {
        if (STS::magnitude(SDM(i, j)) > STS::magnitude(normsB[i] * normsC[j])) {
          om->stream(Warnings)
              << "*** ERROR *** MultiVecTraits::MvTransMv()." << endl
...
bool TestKokkosOperatorTraits(const Teuchos::RCP<OutputManager<ScalarType> > &om,
                              const Teuchos::RCP<const MV> &A,
