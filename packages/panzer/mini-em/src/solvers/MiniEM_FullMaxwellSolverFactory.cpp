...
Teko::InverseLinearOp FullMaxwellSolverFactory::buildInverse(const Teko::LinearOp & lo) const
{
   ...
   auto blo = Teuchos::rcp_const_cast<Thyra::PhysicallyBlockedLinearOpBase<Scalar>>(Teuchos::rcp_dynamic_cast<const Thyra::PhysicallyBlockedLinearOpBase<Scalar>>(lo, true));
   ...
   if (!use_discrete_curl_) {
...

     {
       ...
       return Teuchos::rcp_const_cast<Thyra::LinearOpBase<Scalar>>(Teko::multiply(invU, Teko::toLinearOp(invL)));
     }
   } else {
     Teuchos::TimeMonitor tm(*Teuchos::TimeMonitor::getNewTimer("MaxwellPreconditioner: Block preconditioner"));
...

     {
       ...
       if (!simplifyFaraday_) {
         ...
         return Teuchos::rcp_const_cast<Thyra::LinearOpBase<Scalar>>(Teko::multiply(invU, Teko::multiply(Teko::toLinearOp(invL), Teko::toLinearOp(invDiag))));
       } else
         return Teuchos::rcp_const_cast<Thyra::LinearOpBase<Scalar>>(Teko::multiply(invU, Teko::toLinearOp(invL)));
     }
   }
...
void FullMaxwellSolverFactory::rebuildInverse(const Teko::LinearOp& source, Teko::InverseLinearOp& dest) const {
  dest = buildInverse(source);
}

std::string FullMaxwellSolverFactory::toString() const {
  return "FullMaxwellSolverFactory";
}

Teuchos::RCP<const Teuchos::ParameterList> FullMaxwellSolverFactory::getParameterList() const {
  auto pl = Teuchos::rcp(new Teuchos::ParameterList());
  return pl;
}

//! Initialize from a parameter list
void FullMaxwellSolverFactory::initializeFromParameterList(const Teuchos::ParameterList & pl)
{
   /////////////////////
   // Solver options  //
   // //////////////////
   ...
   invLib = *Teko::InverseLibrary::buildFromStratimikos();
...
#else
       TEUCHOS_TEST_FOR_EXCEPTION(true,std::runtime_error,"ERROR: MiniEM_FullMaxwellSolverFactory: ML is not supported in this build! Requires Epetra support be enabled!");
#endif
     } else {
