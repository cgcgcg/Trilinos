...
*/
#ifndef GALERI_XPETRAUTILS_DEF_HPP
#define GALERI_XPETRAUTILS_DEF_HPP
...
Teuchos::RCP<RealValuedMultiVector>
Utils::CreateCartesianCoordinates(std::string const& coordType, RCP<const Map> const& map, Teuchos::ParameterList& list) {
...
void Utils::getSubdomainData(GlobalOrdinal n, GlobalOrdinal m, GlobalOrdinal i, GlobalOrdinal& start, GlobalOrdinal& end) {
  TEUCHOS_TEST_FOR_EXCEPTION(i >= m, std::runtime_error, "Incorrect input parameter to getSubdomainData: m = " + std::to_string(m) + ", i = " + std::to_string(i));
...
#define GALERI_XPETRAUTILS_INSTANT_XPETRA(S, LO, GO, N) \
  template Teuchos::RCP<::Xpetra::MultiVector<typename Teuchos::ScalarTraits<S>::magnitudeType, LO, GO, N>> \
  Galeri::Xpetra::Utils::CreateCartesianCoordinates<S, LO, GO, ::Xpetra::Map<LO, GO, N>, ::Xpetra::MultiVector<typename Teuchos::ScalarTraits<S>::magnitudeType, LO, GO, N>>(std::string const&, Teuchos::RCP<const ::Xpetra::Map<LO, GO, N>> const&, Teuchos::ParameterList&);

#define GALERI_XPETRAUTILS_INSTANT_TPETRA(S, LO, GO, N) \
  template Teuchos::RCP<Tpetra::MultiVector<typename Teuchos::ScalarTraits<S>::magnitudeType, LO, GO, N>> \
  Galeri::Xpetra::Utils::CreateCartesianCoordinates<S, LO, GO, Tpetra::Map<LO, GO, N>, Tpetra::MultiVector<typename Teuchos::ScalarTraits<S>::magnitudeType, LO, GO, N>>(std::string const&, Teuchos::RCP<const Tpetra::Map<LO, GO, N>> const&, Teuchos::ParameterList&);


#define GALERI_XPETRAUTILS_INSTANT(S, LO, GO, N) \
  GALERI_XPETRAUTILS_INSTANT_XPETRA(S, LO, GO, N) \
  GALERI_XPETRAUTILS_INSTANT_TPETRA(S, LO, GO, N)

#define GALERI_XPETRAUTILS_INSTANT_GO(GO) \
  template void Galeri::Xpetra::Utils::getSubdomainData<GO>(GO, GO, GO, GO&, GO&);

GALERI_XPETRAUTILS_INSTANT_GO(int)
GALERI_XPETRAUTILS_INSTANT_GO(long long)

#endif  // ifndef GALERI_XPETRAUTILS_DEF_HPP
