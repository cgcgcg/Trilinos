...
void fill_with_identity_columns(const MatView<Ordinal, Scalar>& A) {
  deep_copy(A, Scalar{});
...
void generateSingularValues(NormalGenType& magGen,
...
static std::vector<
    typename Teuchos::ScalarTraits<
        typename MatrixViewType::non_const_value_type>::magnitudeType>
...
template <class Ordinal,
          class Scalar,
          class CombineType>
void verifyCombineTemplate(TSQR::Random::NormalGenerator<Ordinal, Scalar>& gen,
...
template <class Ordinal,
          class Scalar>
void verifyCombineTemplateAllCombiners(std::vector<int>& iseed,
                                       const Ordinal numRows,
                                       const Ordinal numCols,
                                       const bool debug) {
  using mag_type =
...
verifyCombineSeqTemplate(TSQR::Random::NormalGenerator<Ordinal, Scalar>& gen,
  ...
  if (debug) {
    cerr << endl
         << "----------------------------------------"
         << endl
         << "TSQR::Combine SequentialTsqr simulation with 2 "
            "cache blocks:"
         << endl
         << "qr( [A1; A2] ), with A1 and A2 "
            "A2 each "
         << numRows << " by " << numCols << endl
         << endl;
  }
  // qr( A1 )
...
void verifyCombine(const int numRows,
                   const int numCols,
                   const bool testReal,
    ...
    if (testReal) {
      {
        ...
        verifyCombineTemplateAllCombiners<int, scalar_type>(iseed, numRows, numCols, debug);
      }
      {
