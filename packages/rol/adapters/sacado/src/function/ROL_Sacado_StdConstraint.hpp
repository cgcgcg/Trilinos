...
public:
  ...
  void value(V &c, const V &x, ROL::Tolerance<Real> &tol ) {
    SV cs = dynamic_cast<SV&>(c);
    const SV xs = dynamic_cast<const SV&>(x);
    value(*(cs.getVector()),*(xs.getVector()),tol);
  }

  void value(vector<Real> &c, const vector<Real>  &x, ROL::Tolerance<Real> &tol) {
    constr_.value(c,x,tol);
  }

  using Constraint<Real>::applyJacobian;
  void applyJacobian( V &jv, const V &v, const V &x, ROL::Tolerance<Real> &tol ) {
  ...
  void applyAdjointJacobian( V &aju, const V &u, const V &x, ROL::Tolerance<Real> &tol ) {
  ...
  void applyAdjointHessian( V &ahuv, const V &u, const V& v, const V &x, ROL::Tolerance<Real> &tol ) {
