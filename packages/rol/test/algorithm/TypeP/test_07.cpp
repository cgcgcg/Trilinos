...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    list.sublist("Step").sublist("Line Search").set("Initial Step Size",0.75);
    list.sublist("Step").sublist("Line Search").set("Maximum Step Size",1e8);
    list.sublist("Step").sublist("Line Search").set("Use Adaptive Step Size Selection",false);
    list.sublist("Step").sublist("Line Search").set("Use Previous Step Length as Initial Guess",false);
    ...
    *outStream << std::endl << "Random Diagonal LASSO Test Problem" << std::endl << std::endl;
    ...
    plist.sublist("General").set("Output Level",iprint);
    plist.sublist("Step").sublist("Line Search").set("User Defined Initial Step Size",true);
    plist.sublist("Step").sublist("Line Search").set("Initial Step Size",0.75);
    plist.sublist("Step").sublist("Line Search").sublist("Descent Method").set("Type","Steepest Descent");
    plist.sublist("Step").sublist("Line Search").sublist("Line-Search Method").set("Type","Backtracking");
    plist.sublist("Status Test").set("Gradient Tolerance",1e-7);
    plist.sublist("Status Test").set("Constraint Tolerance",1e-8);
    plist.sublist("Status Test").set("Step Tolerance",1e-12);
    plist.sublist("Status Test").set("Iteration Limit", 10000);
    ROL::TypeU::LineSearchAlgorithm<RealT> algoLS(plist);
    ROL::Ptr<ROL::Vector<RealT>> solLS = sol->clone();
    solLS->zero();
    algoLS.run(*solLS,*sobj,*outStream);
    solLS->axpy(static_cast<RealT>(-1),*sol);

    err = solLS->norm();
    *outStream << std::endl;
    *outStream << " Absolute Error: " << err << std::endl;
    *outStream << " Relative Error: " << err / sol->norm() << std::endl << std::endl;
