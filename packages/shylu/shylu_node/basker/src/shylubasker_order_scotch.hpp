...
namespace BaskerNS
{
  ...
  template <class Int, class Entry, class Exe_Space>
  int Basker<Int,Entry,Exe_Space>::nested_dissect
  (
   BASKER_MATRIX &M,
   BASKER_TREE &BT
  )
  {
    ...
    return nested_dissect(M, BT, lvls);
  }//end nested_dissect


  //Note that current nested_dissect only works with symmetric matrix
  template <class Int, class Entry, class Exe_Space>
  int Basker<Int,Entry,Exe_Space>::nested_dissect
  (
   BASKER_MATRIX &M,
  ...
  )
  {
    ...
    sg.m = M.nrow;
    sg.cblk = 0;
    sg.permtab = (graph_integral_type *)malloc((sg.m)  *sizeof(graph_integral_type));
    sg.peritab = (graph_integral_type *)malloc((sg.m)  *sizeof(graph_integral_type));
    sg.rangtab = (graph_integral_type *)malloc((num_doms+1)*sizeof(graph_integral_type));
    sg.treetab = (graph_integral_type *)malloc((num_doms+1)*sizeof(graph_integral_type));

    int rval = BASKER_SUCCESS;
    ...
    if (num_levels == 0 && !run_nd_on_leaves && !run_amd_on_leaves) {
      ...
      // Compute Nested-Dissection
      #if defined (HAVE_SHYLU_NODEBASKER_METIS) || defined(HAVE_SHYLU_NODEBASKER_SCOTCH)
      if (Options.use_metis == BASKER_TRUE) {
        // using METIS
        rval = nested_dissect_metis(M, BT, num_domains, sg);
      } else {
        // using SCOTCH
        rval = nested_dissect_scotch(M, BT, num_domains, sg);
      }
      #else
  ...
  )
  {
    ...
    #else
    //----------------------INIT Scotch Graph------------//
    sg.Ap = (graph_integral_type *)malloc((sg.m+1)     *sizeof(graph_integral_type));
    sg.Ai = (graph_integral_type *)malloc((M.nnz)      *sizeof(graph_integral_type));
    ...
    int err;
    graph_integral_type *vwgts; vwgts = NULL;
