...
int main(int argc, char *argv[]) {
  ...
  if (numConverted != 3) {
    ...
    errStr << "Error reading matrix dimensions.  Expected 3 integers, found "
           << numConverted;
  ...
  while (row != i && !done) {
    ...
    for (int k = 0; k < rownnz; k++)
      ofs.write(reinterpret_cast<char *>(&inds[0] + k), sizeof(inds[k]));
    for (int k = 0; k < rownnz; k++)
      ofs.write(reinterpret_cast<char *>(&vals[0] + k), sizeof(vals[k]));
