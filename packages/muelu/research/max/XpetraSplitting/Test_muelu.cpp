...
int main(int argc, char *argv[]) {
  ...
  typedef Xpetra::MatrixSplitting<Scalar, LocalOrdinal, GlobalOrdinal, Node>
      MatrixSplitting;
  typedef Xpetra::CrsMatrixWrap<Scalar, LocalOrdinal, GlobalOrdinal,
                                Xpetra::EpetraNode>
      EpCrsMatrix;
  ...
  // Create Xpetra map
  Teuchos::RCP<const Xpetra::Map<int, GlobalOrdinal, Xpetra::EpetraNode>>
      xpetraMap;
  xpetraMap = Xpetra::MapFactory<int, GlobalOrdinal, Xpetra::EpetraNode>::Build(
      Xpetra::UseEpetra, numGlobalElements, 0, comm);
  ...
  Teuchos::RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
      xpetraMatrix =
          Xpetra::IO<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Read(
              argv[1], Xpetra::UseEpetra, comm);
  // Export matrix from an Xpetra wrapper into an .mtx file
  Xpetra::IO<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Write("A_write.mtx",
                                                               *xpetraMatrix);
  ...
  Teuchos::RCP<MueLu::Hierarchy<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
      Hierarchy = MueLu::CreateXpetraPreconditioner(
          (Teuchos::RCP<Matrix>)xpetraMatrixSplitting, xmlParams);
