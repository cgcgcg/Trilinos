...
int main(int argc, char *argv[]) {
  ...
  typedef Xpetra::MultiVector<Scalar, LocalOrdinal, GlobalOrdinal, Node>
      multivector_type;
  typedef Xpetra::MatrixSplitting<Scalar, LocalOrdinal, GlobalOrdinal, Node,
  ...
  // SplittingDriver
  /*Xpetra::SplittingDriver<Scalar, LocalOrdinal, GlobalOrdinal, Node>
     driver("node.txt", comm); Teuchos::Array<GlobalOrdinal> elementlist =
     driver.GetGlobalRowMap(); driver.printInactive();
        Xpetra::MatrixSplitting<Scalar,LocalOrdinal,GlobalOrdinal,Node,Xpetra::UseTpetra>
     xpetraWrapper( argv[1], argv[2], comm ); std::string
     output_file="A_write.mtx"; xpetraWrapper.writeGlobalMatrix();
        xpetraWrapper.writeRegionMatrices();*/

  // Teuchos::RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node> > A;
  // A = Xpetra::IO<Scalar,LocalOrdinal,GlobalOrdinal,Node>::Read(argv[2],
  // Xpetra::UseTpetra, comm);

  // Create the RegionHandler to deal with mappings of nodes to regions etc.
  Teuchos::RCP<Xpetra::RegionHandler<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
      regionHandler = Teuchos::rcp(
          new Xpetra::RegionHandler<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
              mappingFileName, comm));
  ...
  Teuchos::RCP<tpetra_splitting> matrixSplitting =
      Teuchos::rcp(new tpetra_splitting(matrixFileName, regionHandler, comm));

  // Create region-wise AMG hierarchy
  int max_num_levels = 4;
  int coarsening_factor = 3;
  Xpetra::RegionAMG<Scalar, LocalOrdinal, GlobalOrdinal, Node> preconditioner(
      matrixSplitting, regionHandler, comm, mueluParams, max_num_levels,
      coarsening_factor);

  //  // Setup vectors for test problem
  //  Teuchos::RCP<multivector_type> X = Xpetra::MultiVectorFactory< Scalar,
  //  LocalOrdinal, GlobalOrdinal, Node >::Build(preconditioner.getDomainMap(),
  //  1) ; Teuchos::RCP<multivector_type> Y = Xpetra::MultiVectorFactory<
  //  Scalar, LocalOrdinal, GlobalOrdinal, Node
  //  >::Build(preconditioner.getRangeMap(), 1) ; X->randomize();
