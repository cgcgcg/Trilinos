...
public:
  Problem(Teuchos::ParameterList &list) : list_(list) { SetBoundary(); };
  ...
  // Set methods
  Teuchos::RCP<const Map> setMap(const Teuchos::RCP<const Map> &map) {
    Map_ = map;
  }
...
private:
  void SetBoundary() {
    ...
    if (this->list_.get("left boundary", "Dirichlet") == "Neumann")
      DirichletBC_ ^= DIR_LEFT;
    if (this->list_.get("right boundary", "Dirichlet") == "Neumann")
      DirichletBC_ ^= DIR_RIGHT;
    if (this->list_.get("bottom boundary", "Dirichlet") == "Neumann")
      DirichletBC_ ^= DIR_BOTTOM;
    if (this->list_.get("top boundary", "Dirichlet") == "Neumann")
      DirichletBC_ ^= DIR_TOP;
    if (this->list_.get("front boundary", "Dirichlet") == "Neumann")
      DirichletBC_ ^= DIR_FRONT;
    if (this->list_.get("back boundary", "Dirichlet") == "Neumann")
      DirichletBC_ ^= DIR_BACK;
  }
};
