...
void createBrickMap1D(int numGlobalElements, std::vector<int> &ind,
                      Teuchos::RCP<const Teuchos::Comm<int>> comm) {
  // INPUT: numGlobalElements = number of finite difference nodes along x
  // INPUT: ind = vector containing indices of the rows owned by the current MPI
  // processor INPUT: comm = MPI communicator (MPI_COMM_WORLD)
  ...
  if (mypid != 0 && mypid != comm->getSize() - 1)
    for (int i = 0;
         i <= (static_cast<int>(numGlobalElements / (comm->getSize() - 1))) - 1;
         ++i)
      ind.emplace_back((mypid - 1) * static_cast<int>(numGlobalElements /
                                                      (comm->getSize() - 1)) +
                       i);

  if (mypid == comm->getSize() - 1)
    for (int i = (mypid - 1) *
                 static_cast<int>(numGlobalElements / (comm->getSize() - 1));
         i != numGlobalElements; ++i)
      ind.emplace_back(i);
}

void createBrickMap2D(int nx, int brick_sizex, int brick_sizey,
                      std::vector<int> &ind,
                      Teuchos::RCP<const Teuchos::Comm<int>> comm) {
  ...
  if (mypid != 0) {
    ...
    for (int row = 0; row < brick_sizey; ++row)
      for (int col = brick_sizex * (xpos - 1) + 1; col <= brick_sizex * xpos;
           ++col)
        ind.emplace_back(preliminary + row * nx + col - 1);
  }
}

void createBrickMap3D(int nx, int ny, int brick_sizex, int brick_sizey,
                      int brick_sizez, std::vector<int> &ind,
                      Teuchos::RCP<const Teuchos::Comm<int>> comm) {
  ...
  if (mypid != 0) {
    ...
    int preliminary = nx * ny * brick_sizez * (grid_plane - 1) +
                      nx * brick_sizey * (ypos - 1);
