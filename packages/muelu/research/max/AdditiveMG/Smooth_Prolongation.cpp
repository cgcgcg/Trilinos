...
AdditiveVariant::AdditiveVariant(RCP<crs_matrix_type> A,
                                 RCP<multivector_type> coords,
                                 DomainPartitioning domain) {
...
void AdditiveVariant::AdditiveFineSmoother(RCP<crs_matrix_type> A) {
  ...
  ParameterList &coarse_subdomain_solver =
  ...
  /*B_DD_ = rcp( new Ifpack2::AdditiveSchwarz< row_matrix_type, precond_type
  >(A, 0) ); B_DD_->initialize(); B_DD_->compute();*/
}
...
void AdditiveVariant::AdditiveCoarseSolver(RCP<crs_matrix_type> A) {
  ...
  coarse_list->set("repartition: min rows per proc",
                   static_cast<int>(A->getGlobalNumRows()));
  ...
  ParameterList &coarse_subdomain_solver =
      coarse_smooth_sublist.sublist("subdomain solver parameters");
  coarse_subdomain_solver.set("fact: iluk level-of-fill", 3);
  coarse_subdomain_solver.set("fact: absolute threshold", 0.);
  coarse_subdomain_solver.set("fact: relative threshold", 1.);
  coarse_subdomain_solver.set("fact: relax value", 0.);
...
coarse_file_sublist.set("R", "{1}");*/
  ...
  RCP<xpetra_matrix> mueluA =
      MueLu::TpetraCrs_To_XpetraMatrix<scalar_type, local_ordinal_type,
                                       global_ordinal_type, node_type>(A);

  RCP<Xpetra::MultiVector<scalar_type, local_ordinal_type, global_ordinal_type,
                          node_type>>
      coords = Xpetra::toXpetra(coords_);
  ...
  Teuchos::ArrayRCP<scalar_type> localMV =
      identity_shrunk->getDataNonConst(my_color);
  ...
  RCP<multivector_type> BAP_multivector = rcp(
      new multivector_type(B_fine_->getRangeMap(), AP_shrunk->getNumVectors()));
  RCP<multivector_type> BAP_shrunk = rcp(
      new multivector_type(B_fine_->getRangeMap(), AP_shrunk->getNumVectors()));
  B_fine_->apply(*AP_shrunk, *BAP_shrunk, Teuchos::NO_TRANS,
  ...
  RCP<crs_matrix_type> BAP =
      rcp(new crs_matrix_type(tpetra_prolong->getRowMap(), AP->getColMap(),
  ...
  RCP<crs_matrix_type> Pbar =
      Tpetra::MatrixMatrix::add(1.0, false, *tpetra_prolong, -1.0, false, *BAP);
  RCP<xpetra_matrix> mueluPbar =
      MueLu::TpetraCrs_To_XpetraMatrix<scalar_type, local_ordinal_type,
...
void AdditiveVariant::apply(
    const multivector_type &r, multivector_type &Pr,
    Teuchos::ETransp mode = Teuchos::NO_TRANS,
    scalar_type alpha = Teuchos::ScalarTraits<scalar_type>::one(),
    scalar_type beta = Teuchos::ScalarTraits<scalar_type>::zero()) const {

  Tpetra::Export<local_ordinal_type, global_ordinal_type, node_type>
      Export_fine1(r.getMap(), DomainMap_);
  Tpetra::Export<local_ordinal_type, global_ordinal_type, node_type>
      Export_fine2(RangeMap_, r.getMap());
  ...
  B_Pr_sum.update(1.0, B_Pr1, 1.0, B_Pr2,
                  0.0); // Careful to set the correct coefficients!!!
