...
class Parameters : public Teuchos::VerboseObject<Parameters<GO>>,
                   public Teuchos::Describable {
public:
  Parameters(Teuchos::CommandLineProcessor &clp, GO nx = 16, GO ny = -1,
             GO nz = -1, const std::string &matrixType = "ADR1D",
             int keepBCs = 0, double stretchx = 1.0, double stretchy = 1.0,
             double stretchz = 1.0, double h = 1.0, double delta = 0.0,
             int PMLXL = 0, int PMLXR = 0, int PMLYL = 0, int PMLYR = 0,
             int PMLZL = 0, int PMLZR = 0, double omega = 2.0 * M_PI,
             double shift = 0.5, GO mx = -1, GO my = -1, GO mz = -1,
             int model = 0)
      : nx_(nx), ny_(ny), nz_(nz), mx_(mx), my_(my), mz_(mz),
        stretchx_(stretchx), stretchy_(stretchy), stretchz_(stretchz),
        matrixType_(matrixType), keepBCs_(keepBCs), h_(h), delta_(delta),
        PMLx_left(PMLXL), PMLx_right(PMLXR), PMLy_left(PMLYL),
        PMLy_right(PMLYR), PMLz_left(PMLZL), PMLz_right(PMLZR), omega_(omega),
        shift_(shift), model_(model) {
    ...
    clp.setOption("keepBCs", &keepBCs_,
                  "keep Dirichlet boundary rows in matrix (0=false,1=true)");
    clp.setOption(
        "matrixType", &matrixType_,
        "matrix type: Laplace1D, Laplace2D, Laplace3D, ..."); // TODO: Star2D,
                                                              // numGlobalElements=...
    clp.setOption("h", &h_, "mesh width for uniform h");
    clp.setOption("delta", &delta_, "maximum PML damping value");
    clp.setOption("PMLx_left", &PMLx_left,
                  "PML grid points in x-direction (left boundary)");
    clp.setOption("PMLx_right", &PMLx_right,
                  "PML grid points in x-direction (right boundary)");
    clp.setOption("PMLy_left", &PMLy_left,
                  "PML grid points in y-direction (left boundary)");
    clp.setOption("PMLy_right", &PMLy_right,
                  "PML grid points in y-direction (right boundary)");
    clp.setOption("PMLz_left", &PMLz_left,
                  "PML grid points in z-direction (left boundary)");
    clp.setOption("PMLz_right", &PMLz_right,
                  "PML grid points in z-direction (right boundary)");
  ...
  GO GetNumGlobalElements() const {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(numGlobalElements < 0, std::runtime_error,
                               "Gallery: numGlobalElements < 0 (did you forget "
                               "--ny (or --nz) for 2D (3D) problems?)");
  ...
  std::string description() const {
    ...
    out << "{type = " << GetMatrixType()
        << ", size = " << GetNumGlobalElements() << "} ";
  ...
  describe(Teuchos::FancyOStream &out,
    ...
    if (vl == Teuchos::VERB_MEDIUM || vl == Teuchos::VERB_HIGH ||
        vl == Teuchos::VERB_EXTREME) {
...
private:
  // See Teuchos BUG 5249:
  // https://software.sandia.gov/bugzilla/show_bug.cgi?id=5249
  ...
  // As soon as somebody call GetParameterList(), we freeze all other variables
  // into the list, and ignore them. This allows us to make the modification of
