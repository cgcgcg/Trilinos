...
int coloring2D(int mypid, int ndx) {
  ...
  // detect the y-coordinate in the domain grid associated with the current
  // subdomain
  int ypos = grid_row % 3;
  int xpos;
  int color = -1;

  // detect the x-coordinate in the domain grid associated with current
  // subdomain
  if (0 != mypid % ndx)
    xpos = static_cast<int>(
               mypid - std::floor(static_cast<double>(mypid) / ndx) * ndx) %
           3;
  else
...
int coloring3D(int mypid, int ndx, int ndy) {
  ...
  // detect the row on the current two-dimensional grid where the current
  // subdomain is located
  int plane_row = std::ceil(static_cast<double>(plane_id) / ndx);

  // detect the y-coordinate on the given plane associated with the current
  // subdomain
  ...
  // detect the x-coordinate on the given plane associated with the current
  // subdomain
  if (0 != plane_id % ndx)
    xpos = static_cast<int>(
               mypid - std::floor(static_cast<double>(plane_id) / ndx) * ndx) %
           3;
  else
    xpos = (plane_id - ((ypos - 1) * ndx)) % 3;

  // find a two-dimensional coloring to give to the current subdomain according
  // to its position in the plane where it resides
  if (xpos > 0 && ypos > 0)
  ...
  // use the two-dimensional coloring to determine the three-dimensional
  // coloring
  if (grid_plane % 3 == 1)
    color = label_plane;
