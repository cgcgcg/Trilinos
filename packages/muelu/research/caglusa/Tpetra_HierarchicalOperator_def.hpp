...
removeSmallEntries(
  ...
  using row_ptr_type = typename crs_matrix::local_graph_device_type::
      row_map_type::non_const_type;
  using col_idx_type = typename crs_matrix::local_graph_device_type::
  ...
  Kokkos::parallel_scan(
      ...
      Kokkos::RangePolicy<LocalOrdinal>(0, lclA.numRows()),
      KOKKOS_LAMBDA(const LocalOrdinal rlid, LocalOrdinal &partial_nnz,
                    bool is_final) {
  ...
  auto newA = Teuchos::rcp(
      new crs_matrix(A->getRowMap(), A->getColMap(), rowptr, idx, vals));
  newA->fillComplete(A->getDomainMap(), A->getRangeMap());
  return newA;
}

template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
Teuchos::RCP<Tpetra::BlockedMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
transpose(Teuchos::RCP<Tpetra::BlockedMatrix<Scalar, LocalOrdinal,
                                             GlobalOrdinal, Node>> &A) {

  TEUCHOS_ASSERT(A->ghosted_blockMap_.is_null());

  Teuchos::RCP<Teuchos::ParameterList> transposeParams =
      rcp(new Teuchos::ParameterList);

  Tpetra::RowMatrixTransposer<Scalar, LocalOrdinal, GlobalOrdinal, Node>
      transposerPoint(A->pointA_);
  auto pointAT = transposerPoint.createTranspose(transposeParams);

  Tpetra::RowMatrixTransposer<Scalar, LocalOrdinal, GlobalOrdinal, Node>
      transposerBlock(A->blockA_);
  auto blockAT = transposerBlock.createTranspose(transposeParams);

  auto AT = Teuchos::rcp(
      new Tpetra::BlockedMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
          pointAT, blockAT, A->blockMap_));
...
HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
    HierarchicalOperator(
        const Teuchos::RCP<matrix_type> &nearField,
    ...
    : nearField_(nearField), kernelApproximations_(kernelApproximations),
      basisMatrix_(basisMatrix), transferMatrices_(transferMatrices),
      params_(params) {
  ...
  sendTypeKernelApproximations =
      params_->get<std::string>("Send type kernelApproximations");
  coarseningCriterion_ = params_->get<std::string>("Coarsening criterion");
  TEUCHOS_ASSERT((coarseningCriterion_ == "numClusters") ||
                 (coarseningCriterion_ == "equivalentDense") ||
  ...
  if (doDebugChecks) {
    ...
    TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
        *kernelApproximations_->pointA_->getDomainMap()));
    TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
        *kernelApproximations_->pointA_->getRangeMap()));
    TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
        *kernelApproximations_->pointA_->getRowMap()));

    for (size_t i = 0; i < transferMatrices_.size(); i++) {
      // transfer matrices are entirely local, block diagonal on clusterCoeffMap
      TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
          *transferMatrices_[i]->pointA_->getDomainMap()));
      TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
          *transferMatrices_[i]->pointA_->getColMap()));
      TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
          *transferMatrices_[i]->pointA_->getRowMap()));
      TEUCHOS_ASSERT(clusterCoeffMap_->isSameAs(
          *transferMatrices_[i]->pointA_->getRangeMap()));
    }
  }

  // Set the send types
  Teuchos::RCP<Teuchos::ParameterList> distParams =
      rcp(new Teuchos::ParameterList());
  {
    distParams->set("Send type", sendTypeNearField);
    Teuchos::RCP<const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>>
        nearFieldImporter = nearField_->getGraph()->getImporter();
...

  {
    ...
    Teuchos::RCP<const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>>
        basisMatrixImporter = basisMatrix_->getGraph()->getImporter();
...

  {
    ...
    Teuchos::RCP<const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>>
        kernelApproximationsImporter =
            kernelApproximations_->pointA_->getGraph()->getImporter();
    kernelApproximationsImporter->getDistributor().setParameterList(distParams);
    auto revDistor =
        kernelApproximationsImporter->getDistributor().getReverse(false);
    if (!revDistor.is_null())
      revDistor->setParameterList(distParams);
  }
  ...
  if (setupTransposes) {
    ...
    Tpetra::RowMatrixTransposer<Scalar, LocalOrdinal, GlobalOrdinal, Node>
        transposerBasisMatrix(basisMatrix_);
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::apply(
...
void HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  // upward pass
  {
    Teuchos::TimeMonitor tM(
        *Teuchos::TimeMonitor::getNewTimer(std::string("upward pass")));
    ...
    for (int i = Teuchos::as<int>(transferMatrices_.size()) - 1; i >= 0; i--)
      if (flip) {
        ...
        transferMatrices_[i]->localApply(*coefficients_, *coefficients2_,
                                         Teuchos::NO_TRANS, one, one);
        flip = false;
      } else {
        coefficients_->assign(*coefficients2_);
        transferMatrices_[i]->localApply(*coefficients2_, *coefficients_,
                                         Teuchos::NO_TRANS, one, one);
        flip = true;
      }
  }
  ...
  {
    Teuchos::TimeMonitor tM(
        *Teuchos::TimeMonitor::getNewTimer(std::string("far field 1")));

    RCP<const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>>
        kernelApproximationsImporter =
            kernelApproximations_->pointA_->getGraph()->getImporter();
    if (flip) {
      if (mode == Teuchos::NO_TRANS) {
        coefficients_colmap_->beginImport(
            *coefficients_, *kernelApproximationsImporter, INSERT);
      } else if (mode == Teuchos::TRANS) {
        kernelApproximations_->localApply(*coefficients_, *coefficients_colmap_,
                                          mode, alpha);
        coefficients2_->putScalar(zero);
        coefficients2_->beginExport(*coefficients_colmap_,
                                    *kernelApproximationsImporter, ADD_ASSIGN);
      }
    } else {
      if (mode == Teuchos::NO_TRANS) {
        coefficients_colmap_->beginImport(
            *coefficients2_, *kernelApproximationsImporter, INSERT);
      } else if (mode == Teuchos::TRANS) {
        kernelApproximations_->localApply(*coefficients2_,
                                          *coefficients_colmap_, mode, alpha);
        coefficients_->putScalar(zero);
        coefficients_->beginExport(*coefficients_colmap_,
                                   *kernelApproximationsImporter, ADD_ASSIGN);
      }
    }
  }
  ...
  {
    Teuchos::TimeMonitor tM(
        *Teuchos::TimeMonitor::getNewTimer(std::string("near field 1")));
    if (mode == Teuchos::NO_TRANS) {
  ...
  {
    Teuchos::TimeMonitor tM(
        *Teuchos::TimeMonitor::getNewTimer(std::string("far field 2")));
    ...
    if (flip) {
      if (mode == Teuchos::NO_TRANS) {
        coefficients_colmap_->endImport(*coefficients_,
                                        *kernelApproximationsImporter, INSERT);
        kernelApproximations_->localApply(*coefficients_colmap_,
                                          *coefficients2_, mode, alpha);
      } else if (mode == Teuchos::TRANS) {
        coefficients2_->endExport(*coefficients_colmap_,
                                  *kernelApproximationsImporter, ADD_ASSIGN);
      }
    } else {
      if (mode == Teuchos::NO_TRANS) {
        coefficients_colmap_->endImport(*coefficients2_,
                                        *kernelApproximationsImporter, INSERT);
        kernelApproximations_->localApply(*coefficients_colmap_, *coefficients_,
                                          mode, alpha);
      } else if (mode == Teuchos::TRANS) {
        coefficients_->endExport(*coefficients_colmap_,
                                 *kernelApproximationsImporter, ADD_ASSIGN);
      }
    }
  }
  ...
  {
    Teuchos::TimeMonitor tM(
        *Teuchos::TimeMonitor::getNewTimer(std::string("near field 2")));
  ...
  {
    Teuchos::TimeMonitor tM(
        *Teuchos::TimeMonitor::getNewTimer(std::string("downward pass")));

    for (size_t i = 0; i < transferMatrices_.size(); i++)
      if (flip) {
        coefficients_->assign(*coefficients2_);
        transferMatrices_[i]->localApply(*coefficients2_, *coefficients_,
                                         Teuchos::TRANS, one, one);
        flip = false;
      } else {
        coefficients2_->assign(*coefficients_);
        transferMatrices_[i]->localApply(*coefficients_, *coefficients2_,
...
void HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  // downward pass
  {
    ...
    for (size_t i = 0; i < transferMatrices_.size(); i++)
      if (flip) {
        coefficients_->assign(*coefficients2_);
        transferMatricesT_[i]->localApply(*coefficients2_, *coefficients_,
                                          Teuchos::NO_TRANS, one, one);
        flip = false;
      } else {
        coefficients2_->assign(*coefficients_);
        transferMatricesT_[i]->localApply(*coefficients_, *coefficients2_,
                                          Teuchos::NO_TRANS, one, one);
        flip = true;
      }
    if (flip)
...
Teuchos::RCP<HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
    HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::restrict(
        const Teuchos::RCP<matrix_type> &P) {
  Teuchos::TimeMonitor tM(
      *Teuchos::TimeMonitor::getNewTimer(std::string("Galerkin product")));

  // H_c = P^T * H * P
  using lo_vec_type = typename blocked_map_type::lo_vec_type;
  using vec_type =
      typename Tpetra::Vector<Scalar, LocalOrdinal, GlobalOrdinal, Node>;
  ...
  RCP<matrix_type> newBasisMatrix =
      rcp(new matrix_type(P->getDomainMap(), clusterCoeffMap_, 0));
  ...
  auto ghosted_clusterSizes =
      kernelApproximations_->ghosted_blockMap_->blockSizes_;
  ...
  if ((coarseningCriterion_ == "equivalentDense") ||
    ...
    {
      numUnknownsPerCluster =
          rcp(new vec_type(kernelApproximations_->blockA_->getRowMap(), false));
      auto lcl_clusterSizes =
          clusterSizes->getLocalViewHost(Tpetra::Access::ReadOnly);
      auto lcl_numUnknownsPerCluster =
          numUnknownsPerCluster->getLocalViewHost(Tpetra::Access::OverwriteAll);
      // Compute the transpose of the newBasisMatrix.
      RCP<Tpetra::CrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
          newBasisMatrixT;
      Tpetra::RowMatrixTransposer<Scalar, LocalOrdinal, GlobalOrdinal, Node>
          transposer(newBasisMatrix);
      RCP<Teuchos::ParameterList> transposeParams =
          rcp(new Teuchos::ParameterList);
      ...
      for (LocalOrdinal cluster = 0; cluster < lcl_clusterSizes.extent_int(0);
           ++cluster) {
    ...
    for (int i = Teuchos::as<int>(transferMatrices_.size()) - 1; i >= 0; i--)
      transferMatrices_[i]->blockA_->apply(*numUnknownsPerCluster,
                                           *numUnknownsPerCluster,
                                           Teuchos::NO_TRANS, ONE, ONE);

    // get ghosted numUnknownsPerCluster
    ghosted_numUnknownsPerCluster =
        rcp(new vec_type(ghosted_clusterMap, false));
    auto import = kernelApproximations_->blockA_->getCrsGraph()->getImporter();
    ghosted_numUnknownsPerCluster->doImport(*numUnknownsPerCluster, *import,
                                            Tpetra::INSERT);
  }

  // coarse cluster pair graph
  RCP<matrix_type> newKernelBlockGraph =
      rcp(new matrix_type(kernelApproximations_->blockA_->getCrsGraph()));
  ...
  RCP<matrix_type> diffKernelApprox =
      rcp(new matrix_type(kernelApproximations_->pointA_->getCrsGraph()));
  ...
  {
    typename vec_type::dual_view_type::t_host::const_type
        lcl_numUnknownsPerCluster;
    typename vec_type::dual_view_type::t_host::const_type
        lcl_ghosted_numUnknownsPerCluster;
    auto lcl_offsets =
        Kokkos::create_mirror_view(kernelApproximations_->blockMap_->offsets_);
    auto lcl_ghosted_offsets = Kokkos::create_mirror_view(
        kernelApproximations_->ghosted_blockMap_->offsets_);
    Kokkos::deep_copy(lcl_offsets, kernelApproximations_->blockMap_->offsets_);
    Kokkos::deep_copy(lcl_ghosted_offsets,
                      kernelApproximations_->ghosted_blockMap_->offsets_);
    auto lcl_clusterSizes =
        clusterSizes->getLocalViewHost(Tpetra::Access::ReadOnly);
    auto lcl_ghosted_clusterSizes =
        ghosted_clusterSizes->getLocalViewHost(Tpetra::Access::ReadOnly);

    if ((coarseningCriterion_ == "equivalentDense") ||
        (coarseningCriterion_ == "numClusters")) {
      lcl_numUnknownsPerCluster =
          numUnknownsPerCluster->getLocalViewHost(Tpetra::Access::ReadOnly);
      lcl_ghosted_numUnknownsPerCluster =
          ghosted_numUnknownsPerCluster->getLocalViewHost(
    ...
    if (coarseningCriterion_ == "numClusters") {
      auto lcl_BlockGraph =
          kernelApproximations_->blockA_->getLocalMatrixHost();
      ...
      for (LocalOrdinal brlid = 0; brlid < lcl_BlockGraph.numRows(); ++brlid) {
        ...
        for (LocalOrdinal k = 0; k < brow.length; ++k) {
          ...
          if (brow.value(k) > HALF) {
            ...
            clusterPairSizes.push_back(
                lcl_numUnknownsPerCluster(brlid, 0) *
                lcl_ghosted_numUnknownsPerCluster(bclid, 0));
          }
        }
      }
      std::sort(clusterPairSizes.begin(), clusterPairSizes.end());
      double coarseningRate = Teuchos::as<double>(P->getGlobalNumCols()) /
                              Teuchos::as<double>(P->getGlobalNumRows());
      tgt_clusterPairSize = clusterPairSizes[Teuchos::as<size_t>(
          clusterPairSizes.size() * (1 - coarseningRate))];
      // std::cout << "HERE " << clusterPairSizes[0] << " " <<
      // tgt_clusterPairSize << " " <<
      // clusterPairSizes[clusterPairSizes.size()-1] << std::endl;
    }
    ...
    if (coarseningCriterion_ == "transferLevels") {
      ...
      size_t totalNumClusterPairs =
          kernelApproximations_->blockA_->getGlobalNumEntries();
      RCP<vec_type> tempV = Teuchos::rcp(
          new vec_type(kernelApproximations_->blockMap_->blockMap_, false));
      RCP<vec_type> tempV2 = Teuchos::rcp(
          new vec_type(kernelApproximations_->blockMap_->blockMap_, false));
      ...
      if (keepTransfers == -1) {
        ...
        const double treeCoarseningFactor =
            params_->get<double>("treeCoarseningFactor");
        while (temp >= 2.0) {
          --keepTransfers;
      ...
      for (int k = Teuchos::as<int>(transferMatrices_.size()) - 1; k >= 0;
           --k) {

        size_t clustersInLevel =
            transferMatrices_[k]->blockA_->getGlobalNumEntries();

        if (debugOutput_ && (comm->getRank() == 0))
          std::cout << "level " << k << " clustersInLevel " << clustersInLevel
                    << std::endl;
        ...
        if (keepTransfers >= 0) {
          ...
          doDrop = (droppedClusterPairs + numClusterPairs <
                    (1.0 - coarseningRate) * totalNumClusterPairs);
        }
        if (doDrop) {
          auto lcl_transfer =
              transferMatrices_[k]->blockA_->getLocalMatrixHost();
          auto lcl_transfer_graph = lcl_transfer.graph;
          for (LocalOrdinal j = 0; j < lcl_transfer_graph.entries.extent_int(0);
               j++)
          ...
          if (debugOutput_ && (comm->getRank() == 0))
            std::cout << "Dropped " << transferMatrices_.size() - 1 - k
                      << " transfers of " << transferMatrices_.size()
                      << " dropped cp: " << droppedClusterPairs << std::endl;
          break;
        }
      }
    }
    ...
    auto lcl_KernelApprox =
        kernelApproximations_->pointA_->getLocalMatrixHost();
    ...
    for (LocalOrdinal brlid = 0; brlid < lcl_BlockGraph.numRows(); ++brlid) {
      ...
      for (LocalOrdinal k = 0; k < brow.length; ++k) {
        ...
        if (brow.value(k) > HALF) {
          ...
          if (coarseningCriterion_ == "equivalentDense") {
            // Size of the sparse cluster approximation >= size of dense
            // equivalent
            removeCluster = (brsize * bcsize >=
                             lcl_numUnknownsPerCluster(brlid, 0) *
                                 lcl_ghosted_numUnknownsPerCluster(bclid, 0));
          } else if (coarseningCriterion_ == "numClusters") {
            removeCluster = (lcl_numUnknownsPerCluster(brlid, 0) *
                                 lcl_ghosted_numUnknownsPerCluster(bclid, 0) <
                             tgt_clusterPairSize);
          ...
          if (removeCluster) {
            ...
            // loop over the point matrix and add the entries to
            // diffKernelApprox
            ...
            TEUCHOS_ASSERT_EQUALITY(Teuchos::as<size_t>(row_end - row_start),
                                    brsize);
            TEUCHOS_ASSERT_EQUALITY(Teuchos::as<size_t>(col_end - col_start),
                                    bcsize);
            for (LocalOrdinal rlid = row_start; rlid < row_end; ++rlid) {
              ...
              if (removed != bcsize) {
                ...
                oss << "col_start " << col_start << " col_end " << col_end
                    << std::endl;
                for (LocalOrdinal n = 0; n < row.length; ++n) {
    ...
    if (debugOutput_) {
      ...
      if (comm->getRank() == 0)
        std::cout << "dropped " << gbl_dropped << " kept " << gbl_kept
                  << " ignored " << gbl_ignored << std::endl;
    }
  }

  newKernelBlockGraph->fillComplete(
      kernelApproximations_->blockA_->getDomainMap(),
      kernelApproximations_->blockA_->getRangeMap());
  newKernelBlockGraph = removeSmallEntries(
      newKernelBlockGraph, Teuchos::ScalarTraits<MagnitudeType>::eps());
  diffKernelApprox->fillComplete(clusterCoeffMap_, clusterCoeffMap_);

  // coarse point matrix of cluster pairs
  Teuchos::RCP<matrix_type> newKernelApprox;
  {
    Teuchos::RCP<matrix_type> temp =
        MatrixMatrix::add(ONE, false, *kernelApproximations_->pointA_, -ONE,
                          false, *diffKernelApprox);
    newKernelApprox =
        removeSmallEntries(temp, Teuchos::ScalarTraits<MagnitudeType>::eps());
  }

  // construct identity on clusterCoeffMap_
  Teuchos::RCP<matrix_type> identity =
      buildIdentityMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
          clusterCoeffMap_);

  Teuchos::RCP<blocked_matrix_type> newBlockedKernelApproximation =
      rcp(new blocked_matrix_type(newKernelApprox, newKernelBlockGraph,
                                  kernelApproximations_->blockMap_,
                                  kernelApproximations_->ghosted_blockMap_));
  ...
  std::vector<Teuchos::RCP<blocked_matrix_type>> newTransferMatrices;
  {
    ...
    RCP<vec_type> v_temp =
        rcp(new vec_type(newKernelBlockGraph->getDomainMap()));
    RCP<vec_type> clusterUseCount =
        rcp(new vec_type(newKernelBlockGraph->getDomainMap()));
    v_temp->putScalar(ONE);
    clusterUseCount->putScalar(ZERO);
    newKernelBlockGraph->apply(*v_temp, *clusterUseCount, Teuchos::NO_TRANS);
    newKernelBlockGraph->apply(*v_temp, *clusterUseCount, Teuchos::TRANS, ONE,
                               ONE);
    ...
    for (int i = Teuchos::as<int>(transferMatrices_.size()) - 1; i >= 0; i--) {
      ...
      transferMatrices_[i]->blockA_->localApply(*clusterUseCount, *v_temp,
                                                Teuchos::NO_TRANS);
      Scalar gbl_use_count = v_temp->norm1();
      // if (comm->getRank() == 0)
      //   std::cout << "Transfer " << i << " count " << gbl_use_count <<
      //   std::endl;
      ...
      if (gbl_use_count < HALF) {
        ...
        Teuchos::RCP<matrix_type> temp2 = MatrixMatrix::add(
            ONE, false, *identity, ONE, false, *transferMatrices_[i]->pointA_);
        RCP<matrix_type> temp = rcp(
            new matrix_type(newBasisMatrix->getRowMap(), clusterCoeffMap_, 0));
        MatrixMatrix::Multiply(*newBasisMatrix, false, *temp2, true, *temp);
        newBasisMatrix = temp;
      } else {
        // We keep the i-th transfer for the coarse operator.
        newTransferMatrices.insert(newTransferMatrices.begin(),
                                   transferMatrices_[i]);
      }
    }
  }
  ...
  {
    // transfer = newBasisMatrix * (identity + newTransferMatrices[K-1]^T) * ...
    // * (identity + newTransferMatrices[0])^T
    Teuchos::RCP<matrix_type> transfer = rcp(new matrix_type(*newBasisMatrix));
    for (int i = Teuchos::as<int>(newTransferMatrices.size()) - 1; i >= 0;
         i--) {
      Teuchos::RCP<matrix_type> temp = MatrixMatrix::add(
          ONE, false, *identity, ONE, false, *newTransferMatrices[i]->pointA_);
      Teuchos::RCP<matrix_type> temp2 =
          rcp(new matrix_type(newBasisMatrix->getRowMap(), 0));
    ...
    // newNearField = P^T * nearField * P + diffFarField
    {
      ...
      newNearField =
          MatrixMatrix::add(ONE, false, *temp2, ONE, false, *diffFarField);
      newNearField = removeSmallEntries(
          newNearField, Teuchos::ScalarTraits<MagnitudeType>::eps());
    }
  }

  return Teuchos::rcp(
      new HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
          newNearField, newBlockedKernelApproximation, newBasisMatrix,
          newTransferMatrices, params_));
}

template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
Teuchos::RCP<Tpetra::CrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::toMatrix() {
  Teuchos::TimeMonitor tM(*Teuchos::TimeMonitor::getNewTimer(
      std::string("Conversion from H-matrix to CSR")));
  ...
  if (hasFarField()) {
    ...
    // transfer = basisMatrix_ * (identity + transferMatrices_[K-1]) * ... *
    // (identity + transferMatrices_[0])
    ...
    if (hasTransferMatrices()) {
      ...
      for (int i = Teuchos::as<int>(transferMatrices_.size()) - 1; i >= 0;
           i--) {
        RCP<matrix_type> temp = MatrixMatrix::add(
            ONE, false, *identity, ONE, false, *transferMatrices_[i]->pointA_);
        RCP<matrix_type> temp2 =
            rcp(new matrix_type(basisMatrix_->getRowMap(), 0));
        MatrixMatrix::Multiply(*transfer, false, *temp, true, *temp2);
        transfer = temp2;
      }
    }
    ...
    MatrixMatrix::Multiply(*transfer, false, *kernelApproximations_->pointA_,
                           false, *temp);
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal,
                          Node>::allocateMemory(size_t numVectors) const {
  if (coefficients_.is_null() || coefficients_->getNumVectors() != numVectors) {
    ...
    coefficients_colmap_ = Teuchos::rcp(
        new mv_type(kernelApproximations_->pointA_->getColMap(), numVectors));
  }
}
} // namespace Tpetra
