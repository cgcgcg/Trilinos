...
buildDistanceLaplacian(
  ...
  auto ghosted_coords =
      Xpetra::MultiVectorFactory<Scalar, LocalOrdinal, GlobalOrdinal,
                                 Node>::Build(colMap, coords->getNumVectors());
...

  {
    ...
    auto lcl_ghosted_coords =
        ghosted_coords->getHostLocalView(Xpetra::Access::ReadOnly);
...
constructAuxiliaryOperator(
  ...
  RCP<Xpetra::HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
      hop = rcp_dynamic_cast<Xpetra::HierarchicalOperator<Scalar, LocalOrdinal,
                                                          GlobalOrdinal, Node>>(
          op);

  RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>> auxOp;

  const std::string auxOpStr =
      problemParams.get<std::string>("auxiliary operator");

  if ((auxOpStr == "near") || (auxOpStr == "distanceLaplacian")) {
    if (hop.is_null())
      auxOp = rcp_dynamic_cast<
          Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>(op, true);
    else
    ...
    Xpetra::MatrixUtils<Scalar, LocalOrdinal, GlobalOrdinal,
                        Node>::checkLocalRowMapMatchesColMap(*auxOp);
#endif
    // coordinates
    auto coords =
        Xpetra::IO<typename Teuchos::ScalarTraits<Scalar>::coordinateType,
                   LocalOrdinal, GlobalOrdinal,
                   Node>::ReadMultiVector(problemParams
                                              .get<std::string>("coordinates"),
                                          op->getRangeMap());

    {
      ...
      auto amalgFact = rcp(
          new AmalgamationFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>());
      auto dropFact =
          rcp(new CoalesceDropFactory_kokkos<Scalar, LocalOrdinal,
                                             GlobalOrdinal, Node>());
...

    {
      ...
      fineLevel.Set("A", auxOp);
      auto filterFact =
          rcp(new ThresholdAFilterFactory<Scalar, LocalOrdinal, GlobalOrdinal,
                                          Node>("A", 1.0e-8, true, -1));
    ...
    if (auxOpStr == "distanceLaplacian") {
      ...
      auxOp = buildDistanceLaplacian<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
          graph, coords);
    }
    ...
    auto aux_colmap = IO::ReadMap(problemParams.get<std::string>("aux colmap"),
                                  op->getRangeMap()->lib(),
                                  op->getRangeMap()->getComm(), readBinary);

    auxOp = IOhelpers::Read(auxOpStr, op->getRangeMap(), aux_colmap,
                            op->getRangeMap(), op->getRangeMap(), true,
                            readBinary, readLocal);
  }
...
constructHierarchyFromAuxiliary(
    RCP<Xpetra::Operator<Scalar, LocalOrdinal, GlobalOrdinal, Node>> op,
    RCP<MueLu::Hierarchy<Scalar, LocalOrdinal, GlobalOrdinal, Node>> auxH,
    Teuchos::ParameterList &params, Teuchos::FancyOStream &out) {

  params.set("coarse: max size", 1);
  params.set("max levels", auxH->GetNumLevels());

  const bool implicitTranspose = params.get(
      "transpose: use implicit",
      MueLu::MasterList::getDefault<bool>("transpose: use implicit"));

  auto hop = rcp_dynamic_cast<
      Xpetra::HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>>(
      op);
  if (!hop.is_null())
    op->describe(out, Teuchos::VERB_EXTREME);

  RCP<Hierarchy<Scalar, LocalOrdinal, GlobalOrdinal, Node>> H =
      rcp(new Hierarchy<Scalar, LocalOrdinal, GlobalOrdinal, Node>());
  ...
  for (int lvlNo = 1; lvlNo < auxH->GetNumLevels(); lvlNo++) {
    ...
    RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>> P =
        auxLvl->Get<
            RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>>(
            "P");
    RCP<Xpetra::Operator<Scalar, LocalOrdinal, GlobalOrdinal, Node>> fineAOp =
        fineLvl->Get<
    ...
    if (!implicitTranspose) {
      ...
      RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>> R =
          auxLvl->Get<
              RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>>(
              "R");
      lvl->Set("R", R);
    ...
    auto fineA = rcp_dynamic_cast<Xpetra::HierarchicalOperator<
        Scalar, LocalOrdinal, GlobalOrdinal, Node>>(fineAOp);
    if (!fineA.is_null()) {
...
#ifdef MUELU_HIERARCHICAL_DEBUG
      {
        ...
        const MagnitudeType tol =
            10000 * Teuchos::ScalarTraits<MagnitudeType>::eps();
        auto testLHS = MultiVectorFactory::Build(coarseA->getDomainMap(), 1);
        auto testRHS_HOp_coarse =
            MultiVectorFactory::Build(coarseA->getRangeMap(), 1);
        auto testRHS_HOp_fine =
            MultiVectorFactory::Build(coarseA->getRangeMap(), 1);
      ...
      if ((lvlNo + 1 == auxH->GetNumLevels()) || !coarseA->hasFarField() ||
        ...
        oss << std::left;
        // oss << setw(9) << "rows"  << setw(12) << "nnz"  << setw(14) <<
        // "nnz/row" << setw(12)  << endl;
        oss << setw(9) << numRows << setw(12) << nnz << setw(14) << nnzPerRow
            << endl;
        out << oss.str();
      ...
      // classical RAP
      auto fineAmat = rcp_dynamic_cast<
          Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>(fineAOp,
                                                                     true);
      ...
      coarseLevel.Set("P", P);
      RCP<RAPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>> rapFact =
          rcp(new RAPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>());
      ...
      RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>> matA =
          coarseLevel.Get<
              RCP<Xpetra::Matrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>>(
              "A", rapFact.get());
      ...
      std::ostringstream oss;
      oss << std::left;
      oss << setw(9) << "rows" << setw(12) << "nnz" << setw(14) << "nnz/row"
          << setw(12) << endl;
      oss << setw(9) << numRows << setw(12) << nnz << setw(14) << nnzPerRow
          << endl;
      out << oss.str();
  ...
  RCP<HierarchyManager<Scalar, LocalOrdinal, GlobalOrdinal, Node>>
      mueLuFactory = rcp(new ParameterListInterpreter<Scalar, LocalOrdinal,
                                                      GlobalOrdinal, Node>(
          params, op->getDomainMap()->getComm()));
