...
HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  using TpGOVec =
      TpetraMultiVector<GlobalOrdinal, LocalOrdinal, GlobalOrdinal, Node>;
  ...
  op_ = rcp(new tHOp(
      Teuchos::rcp_dynamic_cast<TpCrs>(
          Teuchos::rcp_dynamic_cast<CrsWrap>(nearField)->getCrsMatrix(), true)
          ->getTpetra_CrsMatrixNonConst(),
      kernelApproximations->getTpetra_BlockedMatrix(),
      Teuchos::rcp_dynamic_cast<TpCrs>(
          Teuchos::rcp_dynamic_cast<CrsWrap>(basisMatrix)->getCrsMatrix(), true)
          ->getTpetra_CrsMatrixNonConst(),
      tTransferMatrices, params));
}
