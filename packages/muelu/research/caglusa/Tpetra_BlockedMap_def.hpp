...
BlockedMap<LocalOrdinal, GlobalOrdinal, Node>::BlockedMap(
    const Teuchos::RCP<const map_type> &pointMap,
    const Teuchos::RCP<lo_vec_type> &blockSizes)
    : pointMap_(pointMap), blockMap_(blockSizes->getMap()),
  ...
  offsets_ =
      Kokkos::View<size_t *>("offsets", blockMap_->getLocalNumElements() + 1);
  auto offsets_h = Kokkos::create_mirror_view(offsets_);
  offsets_h(0) = 0;
  for (size_t blockNum = 0; blockNum < blockMap_->getLocalNumElements();
       ++blockNum) {
  ...
  TEUCHOS_ASSERT_EQUALITY(offsets_h(blockMap_->getLocalNumElements()),
                          pointMap->getLocalNumElements());
