...
public:
  using tHOp =
      Tpetra::HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>;
  ...
  using blocked_matrix_type =
      Xpetra::TpetraBlockedMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>;
  ...
  HierarchicalOperator(
      const RCP<matrix_type> &nearField,
  ...
  /*! Loosely, performs \f$Y = \alpha \cdot A^{\textrm{mode}} \cdot X + \beta
    \cdot Y\f$. However, the details of operation vary according to the values
    of \c alpha and \c beta. Specifically
    - if <tt>beta == 0</tt>, apply() <b>must</b> overwrite \c Y, so that any
    values in \c Y (including NaNs) are ignored.
    - if <tt>alpha == 0</tt>, apply() <b>may</b> short-circuit the operator, so
  ...
  */
  ...
  void residual(const mv_type &X, const mv_type &B, mv_type &R) const {
    Tpetra::Details::residual(*op_, toTpetra(X), toTpetra(B), toTpetra(R));
  }

  RCP<HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>> restrict(
      const RCP<matrix_type> &P) {
    ...
    return Teuchos::rcp(
        new HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
            op_->restrict(
                Teuchos::rcp_dynamic_cast<TpCrs>(
                    Teuchos::rcp_dynamic_cast<CrsWrap>(P)->getCrsMatrix(), true)
  ...
  RCP<matrix_type> toMatrix() {
    ...
    return Teuchos::rcp(new CrsWrap(
        Teuchos::rcp_dynamic_cast<
            CrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>(tpMat)));
  }

  double getCompression() { return op_->getCompression(); }

  RCP<matrix_type> nearFieldMatrix() {
    auto tpMat = Teuchos::rcp(
        new TpetraCrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
            op_->nearFieldMatrix()));
    return Teuchos::rcp(
        new CrsMatrixWrap<Scalar, LocalOrdinal, GlobalOrdinal, Node>(
            Teuchos::rcp_dynamic_cast<
                CrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>>(tpMat)));
  }
  ...
  bool hasFarField() const { return op_->hasFarField(); }

  bool denserThanDenseMatrix() const { return op_->denserThanDenseMatrix(); }
