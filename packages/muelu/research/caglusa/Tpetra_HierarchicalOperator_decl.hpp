...
For H2-matrices, the map also contains the intermediate clusters that might be
needed in upward/downward pass.
...
For H2-matrices:
upward and downward pass in the cluster hierarchy are encoded in transfer
matrices
*/

template <
    class Scalar = Tpetra::Operator<>::scalar_type,
    ...
    class Node = typename Tpetra::Operator<Scalar, LocalOrdinal,
...
public:
  ...
  //! Returns in Y the result of a Tpetra::Operator applied to a
  //! Tpetra::MultiVector X.
  /*!
    \param[in]  X - Tpetra::MultiVector of dimension NumVectors to multiply with
    matrix. \param[out] Y -Tpetra::MultiVector of dimension NumVectors
    containing result.
  */
  void
  ...
  Teuchos::RCP<
      HierarchicalOperator<Scalar, LocalOrdinal, GlobalOrdinal,
                           Node>> restrict(const Teuchos::RCP<matrix_type> &P);
  ...
  double getCompression() {
    ...
    return Teuchos::as<double>(nnz) / (getDomainMap()->getGlobalNumElements() *
                                       getDomainMap()->getGlobalNumElements());
  }

  Teuchos::RCP<matrix_type> nearFieldMatrix() { return nearField_; }
  ...
  size_t getLocalNumRows() const { return nearField_->getLocalNumRows(); }

  size_t getLocalNumCols() const { return nearField_->getLocalNumCols(); }

  GlobalOrdinal getIndexBase() const { return nearField_->getIndexBase(); }

  global_size_t getGlobalNumEntries() const {
    return nearField_->getGlobalNumEntries();
  }

  size_t getLocalNumEntries() const { return nearField_->getLocalNumEntries(); }
  ...
  void getLocalDiagCopy(
      Vector<Scalar, LocalOrdinal, GlobalOrdinal, Node> &diag) const {
  ...
  mag_type getFrobeniusNorm() const { return 0.; }
  ...
  void describe(Teuchos::FancyOStream &out,
    ...
    const size_t nnzKernelApprox =
        kernelApproximations_->pointA_->getGlobalNumEntries();
    const size_t numClusterPairs =
    ...
    const size_t nnzTotal =
        nnzNearField + nnzKernelApprox + nnzBasis + nnzTransfer;
    const double nnzTotalPerRow = Teuchos::as<double>(nnzTotal) / numRows;
    std::ostringstream oss;
    oss << std::left;
    if (printHeader)
      oss << setw(9) << "rows" << setw(12) << "nnz(near)" << setw(14)
          << "nnz(near)/row" << setw(12) << "nnz(basis)" << setw(15)
          << "#cluster pairs" << setw(12) << "nnz(kernel)" << setw(14)
          << "#transfers" << setw(14) << "nnz(transfer)" << setw(12)
          << "nnz(total)" << setw(14) << "nnz(total)/row" << endl;
    oss << setw(9) << numRows << setw(12) << nnzNearField << setw(14)
        << nnzNearPerRow << setw(12) << nnzBasis << setw(15) << numClusterPairs
        << setw(12) << nnzKernelApprox << setw(14) << numTransfers << setw(14)
        << nnzTransfer << setw(12) << nnzTotal << setw(14) << nnzTotalPerRow
        << endl;
    out << oss.str();
  }
  ...
  bool hasTransferMatrices() const { return transferMatrices_.size() > 0; }
