...
public:
  ...
  using tpetra_blocked_matrix_type =
      Tpetra::BlockedMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>;

  TpetraBlockedMatrix(
      const Teuchos::RCP<matrix_type> &pointA,
    ...
    blockMatrix_ = Teuchos::rcp(new tpetra_blocked_matrix_type(
        Teuchos::rcp_dynamic_cast<TpCrs>(
            Teuchos::rcp_dynamic_cast<CrsWrap>(pointA)->getCrsMatrix(), true)
            ->getTpetra_CrsMatrixNonConst(),
        Teuchos::rcp_dynamic_cast<TpCrs>(
            Teuchos::rcp_dynamic_cast<CrsWrap>(blockA)->getCrsMatrix(), true)
            ->getTpetra_CrsMatrixNonConst(),
        blockMap->getTpetra_BlockedMap(), tp_ghosted_blockMap));
  }
  ...
  apply(const Xpetra::MultiVector<Scalar, LocalOrdinal, GlobalOrdinal, Node> &X,
    ...
    blockMatrix_->apply(Xpetra::toTpetra(X), Xpetra::toTpetra(Y), mode, alpha,
                        beta);
  }
  ...
  void localApply(
      ...
      Scalar alpha = Teuchos::ScalarTraits<Scalar>::one(),
      Scalar beta = Teuchos::ScalarTraits<Scalar>::zero()) const {
    blockMatrix_->localApply(Xpetra::toTpetra(X), Xpetra::toTpetra(Y), mode,
                             alpha, beta);
  }
