...
public:
  ...
  using lo_vec_type =
      Xpetra::Vector<LocalOrdinal, LocalOrdinal, GlobalOrdinal, Node>;
  using tpetra_blocked_map_type =
      Tpetra::BlockedMap<LocalOrdinal, GlobalOrdinal, Node>;

  TpetraBlockedMap(const Teuchos::RCP<const map_type> &pointMap,
                   const Teuchos::RCP<lo_vec_type> &blockSizes) {
    using TpLOVec =
        TpetraVector<LocalOrdinal, LocalOrdinal, GlobalOrdinal, Node>;
    tpBlockedMap_ = Teuchos::rcp(new tpetra_blocked_map_type(
        toTpetra(pointMap),
