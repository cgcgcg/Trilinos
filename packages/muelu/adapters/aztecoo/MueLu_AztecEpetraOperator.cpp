...
int AztecEpetraOperator::ApplyInverse(const Epetra_MultiVector &X,
                                      Epetra_MultiVector &Y) const {
  try {
    // There is no rcpFromRef(const T&), so we need to do const_cast
    const Xpetra::EpetraMultiVectorT<GO, NO> eX(
        Teuchos::rcpFromRef(const_cast<Epetra_MultiVector &>(X)));
    ...
    if (X.Values() == Y.Values()) {
      ...
      Teuchos::RCP<Xpetra::EpetraMultiVectorT<GO, NO>> tmpY =
    ...
    std::cerr
        << "Caught an exception in MueLu::AztecEpetraOperator::ApplyInverse():"
        << std::endl
        << e.what() << std::endl;
    return -1;
  }
...
const Epetra_Map &AztecEpetraOperator::OperatorDomainMap() const {
  if (Teuchos::rcp_dynamic_cast<MueLu::RefMaxwell<SC, LO, GO, NO>>(xOp_) !=
      Teuchos::null) {
    RCP<Xpetra::Matrix<SC, LO, GO, NO>> A =
        Teuchos::rcp_dynamic_cast<MueLu::RefMaxwell<SC, LO, GO, NO>>(xOp_)
            ->getJacobian();
  ...
  // TAW there is a problem with the Map version of toEeptra leading to code
  // crashes
