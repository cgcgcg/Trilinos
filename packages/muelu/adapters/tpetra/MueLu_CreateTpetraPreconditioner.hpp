...
//! @file
//! @brief Various adapters that will create a MueLu preconditioner that is a
//! Tpetra::Operator.
...
/*!
  @brief Helper function to create a MueLu or AMGX preconditioner that can be
  used by Tpetra.
  @ingroup MueLuAdapters
  Given a Tpetra::Operator, this function returns a constructed MueLu
  preconditioner.
  @param[in] inA Matrix
...
CreateTpetraPreconditioner(
    const Teuchos::RCP<
        Tpetra::Operator<Scalar, LocalOrdinal, GlobalOrdinal, Node>> &inA,
  ...
  if (inParamList.isParameter(externalMG) &&
      inParamList.get<std::string>(externalMG) == "amgx") {
    const RCP<crs_matrix_type> constCrsA =
        rcp_dynamic_cast<crs_matrix_type>(inA);
    TEUCHOS_TEST_FOR_EXCEPTION(
        constCrsA == Teuchos::null, Exceptions::RuntimeError,
        "CreateTpetraPreconditioner: failed to dynamic cast to "
        "Tpetra::CrsMatrix, which is required to be able to use AmgX.");
  ...
  // Wrap A
  RCP<Matrix> A;
  RCP<block_crs_matrix_type> bcrsA =
      rcp_dynamic_cast<block_crs_matrix_type>(inA);
  ...
  if (crsA != Teuchos::null)
    ...
    RCP<Xpetra::CrsMatrix<SC, LO, GO, NO>> temp =
        rcp(new Xpetra::TpetraBlockCrsMatrix<SC, LO, GO, NO>(bcrsA));
    TEUCHOS_TEST_FOR_EXCEPTION(
        temp == Teuchos::null, Exceptions::RuntimeError,
        "CreateTpetraPreconditioner: cast from Tpetra::BlockCrsMatrix to "
        "Xpetra::TpetraBlockCrsMatrix failed.");
    A = rcp(new Xpetra::CrsMatrixWrap<SC, LO, GO, NO>(temp));
  } else {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true, Exceptions::RuntimeError,
        "CreateTpetraPreconditioner: only Tpetra CrsMatrix and BlockCrsMatrix "
        "types are supported.");
  }
  ...
  if (userList.isParameter("Nullspace")) {
    ...
    try {
      nullspace = TpetraMultiVector_To_XpetraMultiVector<SC, LO, GO, NO>(
          userList.get<RCP<
              Tpetra::MultiVector<Scalar, LocalOrdinal, GlobalOrdinal, Node>>>(
              "Nullspace"));
    } catch (Teuchos::Exceptions::InvalidParameterType &) {
      nullspace = userList.get<
  ...
  RCP<Hierarchy> H =
      MueLu::CreateXpetraPreconditioner<SC, LO, GO, NO>(A, inParamList);
  return rcp(new TpetraOperator<SC, LO, GO, NO>(H));
}

/*!
  @brief Helper function to create a MueLu preconditioner that can be used by
  Tpetra.
  @ingroup MueLuAdapters
...
*/
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void ReuseTpetraPreconditioner(
    const Teuchos::RCP<
        Tpetra::CrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>> &inA,
...
void ReuseTpetraPreconditioner(
    const Teuchos::RCP<
        Tpetra::BlockCrsMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>> &inA,
    MueLu::TpetraOperator<Scalar, LocalOrdinal, GlobalOrdinal, Node> &Op) {
  typedef Scalar SC;
  typedef LocalOrdinal LO;
  typedef GlobalOrdinal GO;
  ...
  RCP<Hierarchy> H = Op.GetHierarchy();
  RCP<Xpetra::CrsMatrix<SC, LO, GO, NO>> temp =
      rcp(new Xpetra::TpetraBlockCrsMatrix<SC, LO, GO, NO>(inA));
  TEUCHOS_TEST_FOR_EXCEPTION(
      temp == Teuchos::null, Exceptions::RuntimeError,
      "ReuseTpetraPreconditioner: cast from Tpetra::BlockCrsMatrix to "
      "Xpetra::TpetraBlockCrsMatrix failed.");
