...
//! @file
//! @brief Various adapters that will create a MueLu preconditioner that is an
//! Epetra_Operator.
...
/*!
  ...
  Given a EpetraCrs_Matrix, this function returns a constructed MueLu
  preconditioner.
  @param[in] inA Matrix
...
CreateEpetraPreconditioner(const Teuchos::RCP<Epetra_CrsMatrix> &inA,
  ...
  if (userList.isParameter("Coordinates")) {
    ...
    try {
      coordinates = EpetraMultiVector_To_XpetraMultiVector<
          typename Teuchos::ScalarTraits<SC>::coordinateType, LO, GO, NO>(
          userList.get<RCP<Epetra_MultiVector>>("Coordinates"));
  ...
  if (userList.isParameter("Nullspace")) {
    ...
    try {
      nullspace = EpetraMultiVector_To_XpetraMultiVector<SC, LO, GO, NO>(
          userList.get<RCP<Epetra_MultiVector>>("Nullspace"));
    } catch (Teuchos::Exceptions::InvalidParameterType &) {
      nullspace =
          userList.get<RCP<Xpetra::MultiVector<SC, LO, GO, NO>>>("Nullspace");
    }
  ...
  RCP<Hierarchy> H =
      MueLu::CreateXpetraPreconditioner<SC, LO, GO, NO>(A, paramListIn);
...
/*!
  ...
  Epetra.
  @ingroup MueLuAdapters
  Given a Epetra_CrsMatrix, this function returns a constructed MueLu
  preconditioner.
  @param[in] inA Matrix
...
void ReuseEpetraPreconditioner(const Teuchos::RCP<Epetra_CrsMatrix> &inA,
