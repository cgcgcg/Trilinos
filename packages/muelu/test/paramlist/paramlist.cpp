...
// ParameterList provides a lot of capabilities but it's not always obvious how
// to use this class properly as there is a lot of important decisions to be
// made.
//
// We want to:
// - provide a list of valid parameters for documentation
// - delegate parameter validation directly to the class that are using the
// parameters.
...
//   (this example is weird because the same list is used for both input
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // Note: users can also directly modify the validParamList list but this is
    // not really useful.
    ...
    std::cout << "# Parameter list after algorithm (flag used/unused):"
              << std::endl;
    if (0) // do not work with my design: flags used/unused are not set for the
