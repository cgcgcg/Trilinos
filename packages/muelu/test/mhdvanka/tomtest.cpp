...
#include <MueLu_UseShortNames.hpp>
  ...
  typedef MueLu::Q2Q1Q2CoarseGridFactory<SC, LO, GO, NO>
  ...
  try {
    ...
    RCP<const Map> AMap = Xpetra::MapFactory<LO, GO>::createUniformContigMap(
        Xpetra::UseTpetra, nv + np + nm, comm);
    ...
    for (size_t PRow = 0; PRow < np; PRow++) {
      ...
      // Insert into A
      A->insertGlobalValues(PRow + nv, newColPtr.view(0, colPtr.size()),
                            valPtr);
    ...
    for (size_t MRow = 0; MRow < nm; MRow++) {
      ...
      // Insert into A
      A->insertGlobalValues(MRow + nv + np, newColPtr.view(0, colPtr.size()),
                            valPtr);
    ...
    RCP<Teuchos::SerialDenseMatrix<GO, GO>> fineGridVElements =
        rcp(new Teuchos::SerialDenseMatrix<GO, GO>(totalFineElements, 18));
    RCP<Teuchos::SerialDenseMatrix<GO, GO>> fineGridPElements =
        rcp(new Teuchos::SerialDenseMatrix<GO, GO>(totalFineElements, 4));
    RCP<Teuchos::SerialDenseMatrix<GO, GO>> fineGridMElements =
        rcp(new Teuchos::SerialDenseMatrix<GO, GO>(totalFineElements, 9));
    ...
    RCP<Q2Q1Q2CoarseGridFactory> coarseElementFact =
        rcp(new Q2Q1Q2CoarseGridFactory());
    RCP<MueLu::MHDRAPFactory<SC, LO, GO, NO>> rapFact =
        rcp(new MueLu::MHDRAPFactory<SC, LO, GO, NO>());
