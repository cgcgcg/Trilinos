...
public:
  ...
  //! Set boolean array indicating which rows correspond to Dirichlet
  //! boundaries.
...
void kernel_coalesce_drop_device(
  ...
  typedef Kokkos::RangePolicy<typename local_matrix_type::execution_space>
      RangePolicy;
  ...
  typename row_map_type::non_const_type rows(
      "row_map", numRows + 1); // rows(0) = 0 automatically
  ...
  Kokkos::parallel_reduce(
      ...
      KOKKOS_LAMBDA(const local_ordinal_type row, local_ordinal_type &nnz) {
        ...
        for (decltype(length) colID = 0; colID < length; colID++) {
          ...
          auto aiiajj = eps * eps * ATS::magnitude(diag(row)) *
                        ATS::magnitude(diag(col)); // eps^2*|a_ii|*|a_jj|
          auto aij2 = ATS::magnitude(rowView.value(colID)) *
                      ATS::magnitude(rowView.value(colID)); // |a_ij|^2
  ...
  Kokkos::parallel_scan(
      ...
      KOKKOS_LAMBDA(const local_ordinal_type i, local_ordinal_type &upd,
                    const bool &final) {
        upd += rows(i);
        if (final)
          rows(i) = upd;
  ...
  LWGraph_kokkos<scalar_type, local_ordinal_type, device_type> graph(
      kokkosGraph);
...
template <class scalar_type, class local_ordinal_type, class device_type>
void kernel_coalesce_drop_serial(
    KokkosSparse::CrsMatrix<scalar_type, local_ordinal_type, device_type> A) {
  typedef KokkosSparse::CrsMatrix<scalar_type, local_ordinal_type, device_type>
      local_matrix_type;
  typedef Kokkos::ArithTraits<scalar_type> ATS;
  ...
  for (int row = 0; row < numRows; row++) {
    ...
    for (decltype(length) colID = 0; colID < length; colID++)
      if ((rowView.colidx(colID) != row) &&
          (ATS::magnitude(rowView.value(colID)) > 1e-13)) {
        boundaryNodes(row) = false;
        break;
      }
  }
  ...
  typename row_map_type::non_const_type rows(
      "row_map", numRows + 1); // rows(0) = 0 automatically
  typename entries_type::non_const_type cols("entries",
                                             20 * numRows); // estimate
  ...
  for (int row = 0; row < numRows; row++) {
    ...
    for (decltype(length) colID = 0; colID < length; colID++) {
      ...
      // Avoid square root by using squared values
      magnitude_type aiiajj = eps * eps * ATS::magnitude(diag(row)) *
                              ATS::magnitude(diag(col)); // eps^2*|a_ii|*|a_jj|
      magnitude_type aij2 = ATS::magnitude(rowView.value(colID)) *
                            ATS::magnitude(rowView.value(colID)); // |a_ij|^2
      ...
      if (rownnz == 1) {
        // If the only element remaining after filtering is diagonal, mark node
        // as boundary
        // FIXME: this should really be replaced by the following
        //    if (indices.size() == 1 && indices[0] == row)
        //        boundaryNodes[row] = true;
...
int main_(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-n") == 0) {
      ...
      std::cout << "./MueLu_KokkosKernels.exe [-n <matrix_size>] [-l "
                   "<number_of_loops>]"
                << std::endl;
      return 0;
    } else {
  ...
  local_matrix_type A =
      kernel_construct<scalar_type, local_ordinal_type, device_type>(n);
