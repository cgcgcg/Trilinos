...
void AlgebraicPermutationStrategy<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  if (A->IsView("stridedMaps")) {
    ...
    nDofsPerNode =
        Teuchos::rcp_dynamic_cast<const StridedMap>(permRowMapStrided)
            ->getFixedBlockSize();
  }
  ...
  // loop over all local rows in matrix A and keep diagonal entries if
  // corresponding matrix rows are not contained in permRowMap
  for (size_t row = 0; row < A->getRowMap()->getLocalNumElements(); row++) {
    GlobalOrdinal grow = A->getRowMap()->getGlobalElement(row);

    if (permRowMap->isNodeGlobalElement(grow) == true)
      continue;
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "MueLu::PermutationFactory::Build: number of nonzeros not equal to "
    ...
    if (grow ==
        gMaxValIdx) // only keep row/col pair if it's diagonal dominant!!!
  ...
  for (typename std::vector<
           std::pair<GlobalOrdinal, GlobalOrdinal>>::const_iterator p =
           keepDiagonalEntries.begin();
       p != keepDiagonalEntries.end(); ++p) {
    gColVec->sumIntoGlobalValue((*p).second, 1.0);
  }

  Teuchos::RCP<Export> exporter =
      ExportFactory::Build(gColVec->getMap(), gDomVec->getMap());
  gDomVec->doExport(*gColVec, *exporter,
                    Xpetra::ADD); // communicate blocked gcolids to all procs
  gColVec->doImport(*gDomVec, *exporter, Xpetra::INSERT);

  std::vector<std::pair<GlobalOrdinal, GlobalOrdinal>>
      permutedDiagCandidatesFiltered; // TODO reserve memory
...

  {
    ...
    for (size_t i = 0; i < permutedDiagCandidates.size(); ++i) {
      ...
      std::pair<GlobalOrdinal, GlobalOrdinal> pp =
          permutedDiagCandidates[permutation[i]];
  ...
  gColVec->doImport(*gDomVec, *exporter,
                    Xpetra::INSERT); // probably not needed // TODO check me

  //*****************************************************************************************
  // first communicate ALL global ids of column indices which are requested by
  // more than one proc to all other procs detect which global col indices are
  // requested by more than one proc and store them in the multipleColRequests
  // vector
  std::vector<GlobalOrdinal>
      multipleColRequests; // store all global column indices from current
                           // processor that are also
  ...
  std::queue<GlobalOrdinal>
      unusedColIdx; // unused column indices on current processor
  ...
  // communicate the global number of column indices which are requested by more
  // than one proc
  LocalOrdinal localMultColRequests =
      Teuchos::as<LocalOrdinal>(multipleColRequests.size());
  ...
  //
  MueLu_sumAll(gDomVec->getMap()->getComm(), (LocalOrdinal)localMultColRequests,
  ...
  if (globalMultColRequests > 0) {
    ...
    // each processor stores how many column ids for exchange are handled by the
    // cur proc
    ...
    for (int i = 0; i < myRank - 1; i++)
      nMyOffset +=
          numGlobalMultColRequests[i]; // calculate offset to store the weights
                                       // on the corresponding place in
                                       // procOverlappingWeights

    const GlobalOrdinal zero = 0;
    std::vector<GlobalOrdinal> procMultRequestedColIds(globalMultColRequests,
                                                       zero);
    std::vector<GlobalOrdinal> global_procMultRequestedColIds(
        globalMultColRequests, zero);

    // loop over all local column GIDs that are also requested by other procs
    for (size_t i = 0; i < multipleColRequests.size(); i++) {
      procMultRequestedColIds[nMyOffset + i] =
          multipleColRequests[i]; // all weights are > 0 ?
    }

    // template ordinal, package (double)
    Teuchos::reduceAll(
        *comm, Teuchos::REDUCE_MAX, Teuchos::as<int>(globalMultColRequests),
        procMultRequestedColIds.data(), global_procMultRequestedColIds.data());
    ...
    for (size_t k = 0; k < global_procMultRequestedColIds.size(); k++) {
      ...
      Teuchos::reduceAll(*comm, Teuchos::REDUCE_MAX, numProcs,
                         MyWeightForColId.data(), GlobalWeightForColId.data());
      ...
      if (gColVec->getMap()->isNodeGlobalElement(globColId)) {
        ...
        // winnerProcRank is unique (even if two procs have the same weight for
        // a column index)

        if (myRank != winnerProcRank) {
          // remove corresponding entry from permutedDiagCandidatesFiltered
          typename std::vector<
              std::pair<GlobalOrdinal, GlobalOrdinal>>::iterator p =
              permutedDiagCandidatesFiltered.begin();
    ...
    }   // end loop over global_procOverlappingWeights and eliminate wrong
        // entries...
  }     // end if globalMultColRequests > 0

  // put together all pairs:
  // size_t sizeRowColPairs = keepDiagonalEntries.size() +
  // permutedDiagCandidatesFiltered.size();
  std::vector<std::pair<GlobalOrdinal, GlobalOrdinal>> RowColPairs;
  RowColPairs.insert(RowColPairs.end(), keepDiagonalEntries.begin(),
                     keepDiagonalEntries.end());
  RowColPairs.insert(RowColPairs.end(), permutedDiagCandidatesFiltered.begin(),
                     permutedDiagCandidatesFiltered.end());
  ...
  gColVec->putScalar(SC_ZERO);
  gDomVec->putScalar(SC_ZERO);
  typename std::vector<std::pair<GlobalOrdinal, GlobalOrdinal>>::iterator pl =
      RowColPairs.begin();
  ...
  for (size_t sz = 0; sz < gDomVec->getLocalLength(); ++sz) {
    Teuchos::ArrayRCP<const Scalar> arrDomVec = gDomVec->getData(0);
    if (arrDomVec[sz] > 1.0) {
      GetOStream(Runtime0) << "RowColPairs has multiple column [" << sz
                           << "]=" << arrDomVec[sz] << std::endl;
    } else if (arrDomVec[sz] == SC_ZERO) {
      GetOStream(Runtime0) << "RowColPairs has empty column [" << sz
                           << "]=" << arrDomVec[sz] << std::endl;
    }
  }
  //&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
#ifdef DEBUG_OUTPUT
  //  for (typename std::vector<std::pair<GlobalOrdinal, GlobalOrdinal>
  //  >::const_iterator p = RowColPairs.begin(); p != RowColPairs.end(); ++p) {
  //    std::cout << "proc: " << myRank << " r/c: " << (*p).first << "/" <<
  //    (*p).second << std::endl;
  //  }
  //    for (typename std::vector<std::pair<GlobalOrdinal, GlobalOrdinal>
  //    >::const_iterator p = RowColPairs.begin(); p != RowColPairs.end(); ++p)
  //    {
  ////      if((*p).first != (*p).second) std::cout << "difference: " <<
  ...
  Teuchos::RCP<Vector> Qperm = VectorFactory::Build(
      A->getDomainMap()); // global variant (based on domain map)
  Teuchos::RCP<Vector> lQperm = VectorFactory::Build(
      A->getColMap()); // local variant (based on column map)
  ...
  Teuchos::RCP<Export> QpermExporter =
      ExportFactory::Build(lQperm->getMap(), Qperm->getMap());

  Teuchos::RCP<Vector> RowIdStatus = VectorFactory::Build(A->getRowMap());
  Teuchos::RCP<Vector> ColIdStatus = VectorFactory::Build(
      A->getDomainMap()); // global variant (based on domain map)
  Teuchos::RCP<Vector> lColIdStatus = VectorFactory::Build(
      A->getColMap()); // local variant (based on column map)
  Teuchos::RCP<Vector> ColIdUsed = VectorFactory::Build(
      A->getDomainMap()); // mark column ids to be already in use
  ...
  // a wide-range permutation is defined as a permutation of rows/columns which
  ...
  {
    Teuchos::ArrayRCP<Scalar> RowIdStatusArray =
        RowIdStatus->getDataNonConst(0);
    Teuchos::ArrayRCP<Scalar> lColIdStatusArray =
        lColIdStatus->getDataNonConst(0);

    typename std::vector<std::pair<GlobalOrdinal, GlobalOrdinal>>::iterator p =
        RowColPairs.begin();
    ...
    if (RowColPairs.size() > 0)
      GetOStream(Warnings0)
          << "MueLu::PermutationFactory: There are Row/Col pairs left!!!"
          << std::endl; // TODO fix me
    ...
    std::queue<GlobalOrdinal>
        qFreeGRowIdx; // store global row ids of "free" rows
    ...
    for (size_t lik = 0; lik < RowIdStatus->getLocalLength(); ++lik) {
      if (RowIdStatusArray[lik] == SC_ZERO) {
        ...
        // detect wide range permutations
        if (floor(qFreeGRowIdx.front() / nDofsPerNode) !=
            floor(RowIdStatus->getMap()->getGlobalElement(lik) /
                  nDofsPerNode)) {
  ...
  std::queue<GlobalOrdinal>
      qUnusedGColIdx; // store global column ids of "free" available columns
  size_t cntFreeColIdx = 0;
  std::queue<GlobalOrdinal>
      qFreeGColIdx; // store global column ids of "free" available columns
  {
    Teuchos::ArrayRCP<Scalar> ColIdStatusArray =
        ColIdStatus->getDataNonConst(0);
    Teuchos::ArrayRCP<Scalar> ColIdUsedArray =
        ColIdUsed->getDataNonConst(0); // not sure about this
    ...
    for (size_t ljk = 0; ljk < ColIdStatus->getLocalLength(); ++ljk) {
      ...
      if (cntUnusedColIdx == 0)
        break;

      if (ColIdStatusArray[ljk] == SC_ZERO) {
        ColIdStatusArray[ljk] = SC_ONE; // use this row id
        Qperm->replaceLocalValue(
            ljk, qUnusedGColIdx
                     .front()); // loop over ColIdStatus (lives on domain map)
        ColIdUsed->replaceGlobalValue(qUnusedGColIdx.front(),
                                      SC_ONE); // ljk is now used, too
        // detect wide range permutations
        if (floor(qUnusedGColIdx.front() / nDofsPerNode) !=
            floor(ColIdStatus->getMap()->getGlobalElement(ljk) /
                  nDofsPerNode)) {
          lWideRangeColPermutations++;
        }
        qUnusedGColIdx.pop();
    ...
    // Qperm->doExport(*lQperm,*QpermExporter,Xpetra::ABSMAX); // no export
    // necessary, since changes only locally
    // ColIdStatus->doExport(*lColIdStatus,*QpermExporter,Xpetra::ABSMAX);

    // count, how many unused column idx are needed on current processor
    // to complete Qperm
    cntFreeColIdx = 0;
    for (size_t ljk = 0; ljk < ColIdStatus->getLocalLength();
         ++ljk) { // TODO avoid this loop
      if (ColIdStatusArray[ljk] == SC_ZERO) {
        cntFreeColIdx++;
      }
    }
  ...
  MueLu_sumAll(comm, Teuchos::as<GlobalOrdinal>(local_cntFreeColIdx),
               global_cntFreeColIdx);
#ifdef DEBUG_OUTPUT
  std::cout << "global # of empty column idx entries in Qperm: "
            << global_cntFreeColIdx << std::endl;
#endif
  ...
  if (global_cntFreeColIdx > 0) {
    ...
    MueLu_sumAll(comm, Teuchos::as<GlobalOrdinal>(local_cntUnusedColIdx),
                 global_cntUnusedColIdx);
#ifdef DEBUG_OUTPUT
    std::cout << "global # of unused column idx: " << global_cntUnusedColIdx
              << std::endl;
#endif
    ...
    // 3) build array of length global_cntUnusedColIdx to globally replicate
    // unused column idx
    std::vector<GlobalOrdinal> local_UnusedColIdxVector(
        Teuchos::as<size_t>(global_cntUnusedColIdx));
    std::vector<GlobalOrdinal> global_UnusedColIdxVector(
        Teuchos::as<size_t>(global_cntUnusedColIdx));
    ...
    for (GlobalOrdinal k = global_cntUnusedColIdxStartIter;
         k < global_cntUnusedColIdxStartIter + local_cntUnusedColIdx; k++) {
      local_UnusedColIdxVector[k] = qUnusedGColIdx.front();
      qUnusedGColIdx.pop();
    }
    Teuchos::reduceAll(
        *comm, Teuchos::REDUCE_MAX, Teuchos::as<int>(global_cntUnusedColIdx),
        local_UnusedColIdxVector.data(), global_UnusedColIdxVector.data());
#ifdef DEBUG_OUTPUT
    ...
    // 5) determine first index in global_UnusedColIdxVector for unused column
    // indices,
...
#ifdef DEBUG_OUTPUT
    GetOStream(Statistics0)
        << "PROC " << myRank << " is allowd to use the following column gids: ";
    for (GlobalOrdinal k = global_UnusedColStartIdx;
         k <
         global_UnusedColStartIdx + Teuchos::as<GlobalOrdinal>(cntFreeColIdx);
         k++) {
    ...
    // 6.) fix Qperm with global entries
    {
      ...
      for (size_t ljk = 0; ljk < ColIdStatus->getLocalLength(); ++ljk) {
        ...
        if (ColIdStatusArray[ljk] == SC_ZERO) {
          ...
          Qperm->replaceLocalValue(
              ljk,
              global_UnusedColIdxVector[global_UnusedColStartIdx + array_iter]);
          ColIdUsed->replaceGlobalValue(
              global_UnusedColIdxVector[global_UnusedColStartIdx + array_iter],
              SC_ONE);
          // detect wide range permutations
          if (floor(global_UnusedColIdxVector[global_UnusedColStartIdx +
                                              array_iter] /
                    nDofsPerNode) !=
              floor(ColIdStatus->getMap()->getGlobalElement(ljk) /
                    nDofsPerNode)) {
  ...
  Teuchos::RCP<CrsMatrixWrap> permPTmatrix =
      Teuchos::rcp(new CrsMatrixWrap(A->getRowMap(), 1));
  Teuchos::RCP<CrsMatrixWrap> permQTmatrix =
      Teuchos::rcp(new CrsMatrixWrap(A->getRowMap(), 1));

  {
    ...
    for (size_t row = 0; row < A->getLocalNumRows(); row++) {
      ...
      Teuchos::ArrayRCP<GlobalOrdinal> indoutP(
          1, Teuchos::as<GO>(Teuchos::ScalarTraits<Scalar>::real(
                 PpermData[row]))); // column idx for Perm^T
      Teuchos::ArrayRCP<GlobalOrdinal> indoutQ(
          1, Teuchos::as<GO>(Teuchos::ScalarTraits<Scalar>::real(
                 QpermData[row]))); // column idx for Qperm
      Teuchos::ArrayRCP<Scalar> valout(1, SC_ONE);
      permPTmatrix->insertGlobalValues(A->getRowMap()->getGlobalElement(row),
                                       indoutP.view(0, indoutP.size()),
                                       valout.view(0, valout.size()));
      permQTmatrix->insertGlobalValues(A->getRowMap()->getGlobalElement(row),
  ...
  for (size_t row = 0; row < permPTmatrix->getLocalNumRows(); row++) {
    if (permPTmatrix->getNumEntriesInLocalRow(row) != 1)
      GetOStream(Warnings0)
          << "#entries in row " << row << " of permPTmatrix is "
          << permPTmatrix->getNumEntriesInLocalRow(row) << std::endl;
    if (permPmatrix->getNumEntriesInLocalRow(row) != 1)
      GetOStream(Warnings0)
          << "#entries in row " << row << " of permPmatrix is "
          << permPmatrix->getNumEntriesInLocalRow(row) << std::endl;
    if (permQTmatrix->getNumEntriesInLocalRow(row) != 1)
      GetOStream(Warnings0)
          << "#entries in row " << row << " of permQmatrix is "
          << permQTmatrix->getNumEntriesInLocalRow(row) << std::endl;
  }

  // build permP * A * permQT
  Teuchos::RCP<Matrix> ApermQt =
      Xpetra::MatrixMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Multiply(
          *A, false, *permQTmatrix, false, GetOStream(Statistics2));
  Teuchos::RCP<Matrix> permPApermQt =
      Xpetra::MatrixMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Multiply(
          *permPmatrix, false, *ApermQt, false, GetOStream(Statistics2));

  /*
    MueLu::Utils<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Write("A.mat", *A);
    MueLu::Utils<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Write("permP.mat",
    *permPmatrix); MueLu::Utils<Scalar, LocalOrdinal, GlobalOrdinal,
    Node>::Write("permQt.mat", *permQTmatrix); MueLu::Utils<Scalar,
    LocalOrdinal, GlobalOrdinal, Node>::Write("permPApermQt.mat",
    *permPApermQt);
  */
  // build scaling matrix
  Teuchos::RCP<Vector> diagVec =
      VectorFactory::Build(permPApermQt->getRowMap(), true);
  Teuchos::RCP<Vector> invDiagVec =
      VectorFactory::Build(permPApermQt->getRowMap(), true);
  Teuchos::RCP<CrsMatrixWrap> diagScalingOp =
      Teuchos::rcp(new CrsMatrixWrap(permPApermQt->getRowMap(), 1));
  {
    ...
    for (size_t i = 0; i < diagVec->getMap()->getLocalNumElements(); ++i) {
      if (diagVecData[i] != SC_ZERO)
        ...
        GetOStream(Statistics0)
            << "MueLu::PermutationFactory: found zero on diagonal in row " << i
            << std::endl;
      }
    }
    ...
    for (size_t row = 0; row < A->getLocalNumRows(); row++) {
      Teuchos::ArrayRCP<GlobalOrdinal> indout(
          1, permPApermQt->getRowMap()->getGlobalElement(
                 row)); // column idx for Perm^T
      Teuchos::ArrayRCP<Scalar> valout(1, invDiagVecData[row]);
      diagScalingOp->insertGlobalValues(A->getRowMap()->getGlobalElement(row),
  ...
  Teuchos::RCP<Matrix> scaledA =
      Xpetra::MatrixMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Multiply(
          *diagScalingOp, false, *permPApermQt, false, GetOStream(Statistics2));
  currentLevel.Set("A", Teuchos::rcp_dynamic_cast<Matrix>(scaledA),
                   genFactory /*this*/);

  currentLevel.Set("permA", Teuchos::rcp_dynamic_cast<Matrix>(permPApermQt),
                   genFactory /*this*/); // TODO careful with this!!!
  currentLevel.Set("permP", Teuchos::rcp_dynamic_cast<Matrix>(permPmatrix),
                   genFactory /*this*/);
  currentLevel.Set("permQT", Teuchos::rcp_dynamic_cast<Matrix>(permQTmatrix),
                   genFactory /*this*/);
  currentLevel.Set("permScaling",
                   Teuchos::rcp_dynamic_cast<Matrix>(diagScalingOp),
                   genFactory /*this*/);

  //// count row permutations
  // count zeros on diagonal in P -> number of row permutations
  Teuchos::RCP<Vector> diagPVec =
      VectorFactory::Build(permPmatrix->getRowMap(), true);
  ...
  // sum up all entries in multipleColRequests over all processors
  MueLu_sumAll(diagPVec->getMap()->getComm(),
               Teuchos::as<GlobalOrdinal>(lNumRowPermutations),
               gNumRowPermutations);

  //// count column permutations
  // count zeros on diagonal in Q^T -> number of column permutations
  Teuchos::RCP<Vector> diagQTVec =
      VectorFactory::Build(permQTmatrix->getRowMap(), true);
  ...
  // sum up all entries in multipleColRequests over all processors
  MueLu_sumAll(diagQTVec->getMap()->getComm(),
               Teuchos::as<GlobalOrdinal>(lNumColPermutations),
               gNumColPermutations);

  currentLevel.Set("#RowPermutations", gNumRowPermutations,
                   genFactory /*this*/);
  currentLevel.Set("#ColPermutations", gNumColPermutations,
                   genFactory /*this*/);
  currentLevel.Set("#WideRangeRowPermutations", gWideRangeRowPermutations,
                   genFactory /*this*/);
  currentLevel.Set("#WideRangeColPermutations", gWideRangeColPermutations,
                   genFactory /*this*/);

  GetOStream(Statistics0) << "#Row    permutations/max possible permutations: "
                          << gNumRowPermutations << "/"
                          << diagPVec->getMap()->getGlobalNumElements()
                          << std::endl;
  GetOStream(Statistics0) << "#Column permutations/max possible permutations: "
                          << gNumColPermutations << "/"
                          << diagQTVec->getMap()->getGlobalNumElements()
                          << std::endl;
  GetOStream(Runtime1) << "#wide range row permutations: "
                       << gWideRangeRowPermutations
                       << " #wide range column permutations: "
                       << gWideRangeColPermutations << std::endl;
}
