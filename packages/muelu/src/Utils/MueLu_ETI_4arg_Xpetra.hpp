...
// The macros "MUELU_ETI_GROUP_EPETRA" and "MUELU_ETI_GROUP_TPETRA" must be
...
#if (defined(HAVE_MUELU_EPETRA) && defined(EPETRA_HAVE_OMP) &&                 \
     (!defined(HAVE_TPETRA_INST_OPENMP) ||                                     \
      !defined(HAVE_TPETRA_INST_INT_INT)))
// Epetra is enabled with OpenMP node, but Tpetra is a) not enabled, or b) is
// not instantiated on OpenMP, or c) is not instantiated on OpenMP with
// <double,int,int>
typedef Tpetra::KokkosCompat::KokkosOpenMPWrapperNode EpetraNode;
#elif (                                                                        \
    defined(HAVE_MUELU_EPETRA) && !defined(EPETRA_HAVE_OMP) &&                 \
    (!defined(HAVE_TPETRA_INST_SERIAL) || !defined(HAVE_TPETRA_INST_INT_INT)))
// Epetra is enabled with Serial node, but Tpetra is a) not enabled, or b) is
// not instantiated on Serial, or c) is not instantiated on Serial with
// <double,int,int>
