...
struct CompPairs {
  ...
  bool operator()(LocalOrdinal a, LocalOrdinal b) {
    ...
    return Teuchos::ScalarTraits<Scalar>::magnitude(vinternal_[a]) >
           Teuchos::ScalarTraits<Scalar>::magnitude(vinternal_[b]);
  }
};
...
template <class Scalar, class LocalOrdinal>
void sortingPermutation(const std::vector<Scalar> &values,
  ...
  std::sort(v.begin(), v.end(),
            MueLu::CreateCmpPairs<Scalar, LocalOrdinal>(values));
}
...
public:
   ...
   * \param genFactory: const pointer to generating (calling) PermutationFactory
   ...
   // Decide which variables have to be generated, give them back per reference
   to the PermutationFactory.
   */
