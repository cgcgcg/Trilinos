...
void LocalPermutationStrategy<Scalar, LocalOrdinal, GlobalOrdinal,
                              Node>::BuildPermutations(size_t nDofsPerNode)
    const {
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void LocalPermutationStrategy<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  Teuchos::SerialDenseMatrix<LocalOrdinal, Scalar> subBlockMatrix(
      nDofsPerNode, nDofsPerNode, true);
  std::vector<GlobalOrdinal> growIds(nDofsPerNode);

  // loop over local nodes
  // TODO what about nOffset?
  LocalOrdinal numLocalNodes =
      A->getRowMap()->getLocalNumElements() / nDofsPerNode;
  for (LocalOrdinal node = 0; node < numLocalNodes; ++node) {
    ...
    for (LocalOrdinal lrdof = 0; lrdof < lonDofsPerNode; ++lrdof) {
      ...
      for (size_t j = 0; j < Teuchos::as<size_t>(indices.size()); j++) {
        ...
        GlobalOrdinal gcnodeid =
            globalDofId2globalNodeId(A, gcol); // -> global node id
        if (grnodeid == gcnodeid) {
          if (maxVal != Teuchos::ScalarTraits<MT>::zero()) {
            subBlockMatrix(lrdof, gcol % nDofsPerNode) = vals[j] / maxVal;
          } else {
            subBlockMatrix(lrdof, gcol % nDofsPerNode) =
                vals[j]; // there is a problem
    ...
    std::vector<Scalar> performance_vector =
        std::vector<Scalar>(result_permvecs_.size());
    ...
    for (size_t j = 0; j < Teuchos::as<size_t>(performance_vector.size());
         j++) {
      if (Teuchos::ScalarTraits<Scalar>::magnitude(performance_vector[j]) >
          maxVal) {
        maxVal =
            Teuchos::ScalarTraits<Scalar>::magnitude(performance_vector[j]);
        maxPerformancePermutationIdx = j;
      }
    }

    // build RowColPairs for best permutation
    std::vector<int> bestPerformancePermutation =
        result_permvecs_[maxPerformancePermutationIdx];
    for (size_t t = 0; t < nDofsPerNode; t++) {
      RowColPairs.push_back(
          std::make_pair(growIds[t], growIds[bestPerformancePermutation[t]]));
    }
  ...
  if (RowColPairs.size() > 0)
    GetOStream(Warnings0)
        << "MueLu::LocalPermutationStrategy: There are Row/col pairs left!"
        << std::endl;
  ...
  Teuchos::RCP<Matrix> ApermQt =
      Xpetra::MatrixMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Multiply(
          *A, false, *permQTmatrix, false, GetOStream(Statistics2), true, true);
  Teuchos::RCP<Matrix> permPApermQt =
      Xpetra::MatrixMatrix<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Multiply(
          *permPmatrix, false, *ApermQt, false, GetOStream(Statistics2), true,
          true);
  ...
  *permPmatrix); MueLu::Utils<Scalar, LocalOrdinal, GlobalOrdinal,
  Node>::Write("permQt.mat", *permQTmatrix); MueLu::Utils<Scalar, LocalOrdinal,
  GlobalOrdinal, Node>::Write("permPApermQt.mat", *permPApermQt);
  */
  ...
  for (size_t i = 0; i < diagVec->getMap()->getLocalNumElements(); ++i) {
    if (diagVecData[i] != SC_ZERO)
      ...
      lCntZeroDiagonals++;
      // GetOStream(Statistics0) << "MueLu::LocalPermutationStrategy: found zero
      // on diagonal in row " << i << std::endl;
    }
  }
  ...
  MueLu_sumAll(diagPVec->getMap()->getComm(), lNumRowPermutations,
               gNumRowPermutations);
  ...
  // sum up all entries in multipleColRequests over all processors
  MueLu_sumAll(diagQTVec->getMap()->getComm(), lNumColPermutations,
               gNumColPermutations);
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
GlobalOrdinal
LocalPermutationStrategy<Scalar, LocalOrdinal, GlobalOrdinal,
                         Node>::getGlobalDofId(const Teuchos::RCP<Matrix> &A,
                                               LocalOrdinal localNodeId,
                                               LocalOrdinal localDof) const {
  size_t nDofsPerNode = 1;
  if (A->IsView("stridedMaps")) {
    Teuchos::RCP<const Map> permRowMapStrided = A->getRowMap("stridedMaps");
    nDofsPerNode =
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
GlobalOrdinal
LocalPermutationStrategy<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
    globalDofId2globalNodeId(const Teuchos::RCP<Matrix> &A,
                             GlobalOrdinal grid) const {
  size_t nDofsPerNode = 1;
  if (A->IsView("stridedMaps")) {
    Teuchos::RCP<const Map> permRowMapStrided = A->getRowMap("stridedMaps");
    nDofsPerNode =
  ...
  return (GlobalOrdinal)grid /
         (GlobalOrdinal)nDofsPerNode; // TODO what about nOffset???
}
