...
RepartitionInterface<LocalOrdinal, GlobalOrdinal, Node>::GetValidParameterList()
    const {
  ...
  validParamList->set<RCP<const FactoryBase>>(
      ...
      "(advanced) Factory generating the AmalgamatedPartition (e.g. an "
      "IsorropiaInterface)");
...
template <class LocalOrdinal, class GlobalOrdinal, class Node>
void RepartitionInterface<LocalOrdinal, GlobalOrdinal, Node>::DeclareInput(
...
template <class LocalOrdinal, class GlobalOrdinal, class Node>
void RepartitionInterface<LocalOrdinal, GlobalOrdinal, Node>::Build(
    Level &level) const {
  FactoryMonitor m(*this, "Build", level);

  RCP<Matrix> A = Get<RCP<Matrix>>(level, "A");
  RCP<Xpetra::Vector<GO, LO, GO, NO>> amalgPartition =
      Get<RCP<Xpetra::Vector<GO, LO, GO, NO>>>(level, "AmalgamatedPartition");
  int numParts = Get<int>(level, "number of partitions");

  RCP<const Map> rowMap = A->getRowMap();

  // standard case: use matrix info and amalgamated rebalancing info to create
  // "Partition" vector
  RCP<const Teuchos::Comm<int>> comm = A->getRowMap()->getComm();

  // Short cut: if we only need one partition, then create a dummy partition
  // vector
  ...
  LO stridedblocksize =
      blockdim; // size of strided block id "blockid" (default = fullblocksize,
                // only if blockid!=-1 stridedblocksize <= fullblocksize)
  ...
  if (A->IsView("stridedMaps") &&
    ...
    // GetOStream(Statistics0, -1) << "RepartitionInterface::Build():" << "
    // found blockdim=" << blockdim << " from strided maps (blockid=" << blockid
    // << ", strided block size=" << stridedblocksize << "). offset=" << offset
    // << std::endl;
  } else
    GetOStream(Statistics0, -1)
        << "RepartitionInterface::Build(): no striding information available. "
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      Teuchos::as<int>(nodeMap->getLocalNumElements()) * stridedblocksize !=
          Teuchos::as<int>(rowMap->getLocalNumElements()),
      Exceptions::RuntimeError,
      "Inconsistency between nodeMap and dofMap: we are supporting block maps "
      "only. No support for general strided maps, yet!");

  // RCP<std::map<GO,std::vector<GO> > > nodegid2dofgids =
  // amalgInfo->GetGlobalAmalgamationParams();
  ...
  for (size_t i = 0; i < nodeMap->getLocalNumElements(); i++) {
    // not fully sure about this. We're filling local ids in the decomposition
    // vector with the results stored in array. The decomposition vector is
    // created using the rowMap of A
    ...
    for (LO j = 0; j < stridedblocksize /*DOFs.size()*/; j++) {
      ...
      // LO lDofId = rowMap->getLocalElement(DOFs[j]);     // -> i doubt that we
      // need this!
