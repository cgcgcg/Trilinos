...
IsorropiaInterface<LocalOrdinal, GlobalOrdinal, Node>::GetValidParameterList()
    const {
...
template <class LocalOrdinal, class GlobalOrdinal, class Node>
void IsorropiaInterface<LocalOrdinal, GlobalOrdinal, Node>::DeclareInput(
...
template <class LocalOrdinal, class GlobalOrdinal, class Node>
void IsorropiaInterface<LocalOrdinal, GlobalOrdinal, Node>::Build(
    Level &level) const {
  ...
  RCP<AmalgamationInfo> amalInfo =
      Get<RCP<AmalgamationInfo>>(level, "UnAmalgamationInfo");
  ...
  if (numParts == 1 || numParts == -1) {
    ...
    RCP<Xpetra::Vector<GO, LO, GO, NO>> decomposition =
        Xpetra::VectorFactory<GO, LO, GO, NO>::Build(rowMap, true);
  ...
  // 1) the "Graph" for the aggregates is meant to be based on the rebalanced
  // matrix A 2) we cannot call a CoalesceDropFactory::Build here since this is
  // very complicated and
  //    completely messes up the whole factory chain
  // 3) CoalesceDropFactory is meant to provide some minimal Graph information
  // for the aggregation
  ...
  LO nStridedOffset =
      0; // DOF offset for strided block id "blockid" (default = 0)
  // LO stridedblocksize = blockdim; // size of strided block id "blockid"
  // (default = fullblocksize, only if blockid!=-1 stridedblocksize <=
  // fullblocksize)
  ...
  if (A->IsView("stridedMaps") &&
    ...
    // GetOStream(Statistics0) << "IsorropiaInterface::Build():" << " found
    // blockdim=" << blockdim << " from strided maps (blockid=" << blockid << ",
    // strided block size=" << stridedblocksize << "). offset=" << offset <<
    // std::endl;
  } else
    GetOStream(Statistics0)
        << "IsorropiaInterface::Build(): no striding information available. "
  ...
  RCP<const BlockedMap> bnodeMap =
      Teuchos::rcp_dynamic_cast<const BlockedMap>(nodeMap);
  if (!bnodeMap.is_null())
    nodeMap = bnodeMap->getMap();

  GetOStream(Statistics0) << "IsorropiaInterface:Build(): nodeMap "
                          << nodeMap->getLocalNumElements() << "/"
                          << nodeMap->getGlobalNumElements() << " elements"
                          << std::endl;

  // 3) create graph of amalgamated matrix
  RCP<CrsGraph> crsGraph =
      CrsGraphFactory::Build(nodeMap, A->getLocalMaxNumRowEntries() * blockdim);
  ...
  for (LO row = 0; row < Teuchos::as<LO>(A->getRowMap()->getLocalNumElements());
    ...
    // JHU 2019-20-May this is identical to
    // AmalgamationFactory::DOFGid2NodeId(), and is done to break a circular
    // dependence when libraries are built statically
  ...
  */
...
#ifdef HAVE_MUELU_EPETRA
  RCP<Xpetra::EpetraCrsGraphT<GO, Node>> epCrsGraph =
      Teuchos::rcp_dynamic_cast<Xpetra::EpetraCrsGraphT<GO, Node>>(crsGraph);
  if (epCrsGraph != Teuchos::null) {
    RCP<const Epetra_CrsGraph> epetraCrsGraph =
        epCrsGraph->getEpetra_CrsGraph();

    RCP<Isorropia::Epetra::Partitioner> isoPart = Teuchos::rcp(
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        size != Teuchos::as<int>(nodeMap->getLocalNumElements()),
        Exceptions::RuntimeError,
        "length of array returned from extractPartsView does not match local "
        "length of rowMap");

    RCP<Xpetra::Vector<GO, LO, GO, NO>> decomposition =
        Xpetra::VectorFactory<GO, LO, GO, NO>::Build(nodeMap, false);
  ...
  RCP<Xpetra::TpetraCrsGraph<LO, GO, Node>> tpCrsGraph =
      Teuchos::rcp_dynamic_cast<Xpetra::TpetraCrsGraph<LO, GO, Node>>(crsGraph);
  TEUCHOS_TEST_FOR_EXCEPTION(tpCrsGraph != Teuchos::null,
                             Exceptions::RuntimeError,
                             "Tpetra is not supported with Isorropia.");
#else
  TEUCHOS_TEST_FOR_EXCEPTION(false, Exceptions::RuntimeError,
                             "Isorropia is an interface to Zoltan which only "
                             "has support for LO=GO=int and SC=double.");
  ...
  // TEUCHOS_TEST_FOR_EXCEPTION(!level.IsAvailable("AmalgamatedPartition"),
  // Exceptions::RuntimeError, "IsorropiaInterface::Build : no \'Partition\'
  // vector available on level. Isorropia failed to build a partition of the
  // non-repartitioned graph of A. Please make sure, that Isorropia is correctly
  // compiled (Epetra/Tpetra).");
