...
/*!
  ...
  Interface to Isorropia allowing to access other rebalancing/repartitioning
  algorithms from Zoltan than RCB This includes methods (like PHG) which do not
  rely on user-provided coordinate or mesh information. This class produces
  node-based rebalancing information (stored in "AmalgamatedPartition") which is
  used as input for the RepartitionInterface class.

  It tries to consider the "number of partitions" variable when repartitioning
  the system.
  ...
  process | | UnAmalgamationInfo | Factory |   null |  | * | * | Generating
  factory of UnAmalgamationInfo | number of partitions                   | GO |
  - |  |  |  | Short-cut parameter set by RepartitionFactory. Avoid
  repartitioning algorithms if only one partition is necessary (see details
  below)
  ...
  IsorropiaInterface::GetValidParameterList).<br> The * in the @c requested
  column states that the data is requested as input with all dependencies (see
  IsorropiaInterface::DeclareInput).
  ...
  ----------|--------------|------------
  | AmalgamatedPartition | IsorropiaInterface   | GOVector based on the node map
...
class IsorropiaInterface : public SingleLevelFactoryBase {
  ...
  typedef double Scalar; // FIXME This class only works with the Epetra stack,
                         // i.e., Scalar = double
