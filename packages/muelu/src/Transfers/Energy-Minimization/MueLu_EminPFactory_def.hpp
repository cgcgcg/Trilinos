...
EminPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::GetValidParameterList()
    const {
...
#define SET_VALID_ENTRY(name)                                                  \
  ...
  SET_VALID_ENTRY("emin: iterative method");
  {
    ...
    validParamList->getEntry("emin: iterative method")
        .setValidator(rcp(
            new validatorType(Teuchos::tuple<std::string>("cg", "sd", "gmres"),
...
#undef SET_VALID_ENTRY
  ...
  validParamList->set<RCP<const FactoryBase>>(
      "P", Teuchos::null, "Generating factory for the initial guess");
  validParamList->set<RCP<const FactoryBase>>(
      "Constraint", Teuchos::null, "Generating factory for constraints");
  ...
  validParamList->set<RCP<Constraint>>("Constraint0", Teuchos::null,
                                       "Initial Constraint");
  validParamList->set<bool>("Keep Constraint0", false,
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void EminPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::DeclareInput(
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void EminPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Build(
    Level &fineLevel, Level &coarseLevel) const {
  BuildP(fineLevel, coarseLevel);
}

template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void EminPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::BuildP(
    Level &fineLevel, Level &coarseLevel) const {
  ...
  if (!P->IsView("stridedMaps")) {
    if (A->IsView("stridedMaps") == true) {
      ...
      RCP<const StridedMap> dMap = StridedMapFactory::Build(
          X->GetPattern()->getDomainMap(), stridingInfo);
