...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
TopRAPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::TopRAPFactory(
    RCP<const FactoryManagerBase> /* parentFactoryManagerFine */,
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void TopRAPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::DeclareInput(
    Level & /* fineLevel */, Level &coarseLevel) const {
  if (PFact_ != Teuchos::null)
    coarseLevel.DeclareInput("P", PFact_.get());
  if (RFact_ != Teuchos::null)
    coarseLevel.DeclareInput("R", RFact_.get());
  if ((AcFact_ != Teuchos::null) && (AcFact_ != NoFactory::getRCP()))
    coarseLevel.DeclareInput("A", AcFact_.get());
}

template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void TopRAPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Build(
    Level & /* fineLevel */, Level &coarseLevel) const {
  if ((PFact_ != Teuchos::null) && (PFact_ != NoFactory::getRCP())) {
    RCP<Operator> oP = coarseLevel.Get<RCP<Operator>>("P", PFact_.get());
    // Don't have a valid operator (e.g., # global aggregates is 0) so we just
    // bail out This level will ultimately be removed in MueLu_Hierarchy_defs.h
    // via a resize()j
    if (oP == Teuchos::null)
      return;
    ...
    coarseLevel.AddKeepFlag(
        "P", NoFactory::get(),
        MueLu::Final); // FIXME2: Order of Remove/Add matter (data removed
                       // otherwise). Should do something about this
    coarseLevel.RemoveKeepFlag(
        "P", NoFactory::get(),
        MueLu::UserData); // FIXME: This is a hack, I should change behavior of
                          // Level::Set() instead. FIXME3: Should not be removed
                          // if flag was there already
  }
  ...
  if ((RFact_ != Teuchos::null) && (RFact_ != NoFactory::getRCP())) {
    ...
    coarseLevel.RemoveKeepFlag("R", NoFactory::get(),
                               MueLu::UserData); // FIXME: This is a hack
  }
  ...
  if ((AcFact_ != Teuchos::null) && (AcFact_ != NoFactory::getRCP())) {
    ...
    coarseLevel.RemoveKeepFlag("A", NoFactory::get(),
                               MueLu::UserData); // FIXME: This is a hack
  }
}
} // namespace MueLu
