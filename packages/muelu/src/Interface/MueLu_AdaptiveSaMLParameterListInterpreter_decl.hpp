...
/*!
  ...
  @brief Class that accepts ML-style parameters and builds a MueLu
  preconditioner. This interpreter uses the same default values as ML. This
  allows to compare ML/MueLu results
*/
...
public:
  ...
  //!
  //! The factories in factoryList allow the user to add user-specific factories
  //! to the MueLu Hierarchy. The idea is to be able to add some factories that
  //! write out some debug information etc. which are not handled by the ML
  //! Parameter List itself. See information about the
  //! RAPFactory::AddTransferFactory method, too!
  AdaptiveSaMLParameterListInterpreter(
      Teuchos::ParameterList &paramList,
      std::vector<RCP<FactoryBase>> factoryList =
          std::vector<RCP<FactoryBase>>(0));
  ...
  //@{
  /*! @brief Add transfer factory in the end of list of transfer factories for
  RAPFactory.

  This allows the user to add user-specific factories to the MueLu Hierarchy.
  The idea is to be able to add some factories that write out some debug
  information etc. which are not handled by the ML Parameter List itself. See
  information about the RAPFactory::AddTransferFactory method, too!
  */
...
private:
  ...
  //! internal routine to add a new factory manager used for the initialization
  //! phase
  void AddInitFactoryManager(int startLevel, int numDesiredLevel,
                             RCP<FactoryManagerBase> manager) {
    const int lastLevel = startLevel + numDesiredLevel - 1;
    if (init_levelManagers_.size() < lastLevel + 1)
      init_levelManagers_.resize(lastLevel + 1);
  ...
  //! hierarchy->Setup()
  Teuchos::RCP<FactoryManagerBase> InitLvlMngr(int levelID,
                                               int lastLevelID) const {

    // Please not that the order of the 'if' statements is important.

    if (levelID == -1)
      return Teuchos::null; // when this routine is called with levelID == '-1',
                            // it means that we are processing the finest Level
                            // (there is no finer level)
    if (levelID == lastLevelID + 1)
      return Teuchos::null; // when this routine is called with levelID ==
                            // 'lastLevelID+1', it means that we are processing
                            // the last level (ie: there is no nextLevel...)
    ...
    if (levelID >= init_levelManagers_.size())
      return init_levelManagers_[init_levelManagers_.size() -
                                 1]; // last levelManager is used for all the
                                     // remaining levels.
  ...
  //! export aggregates
  bool bExportAggregates_; //!< if set to true an AggregationExportFactory is
                           //!< used to export aggregation information (default
                           //!< = false)

  //! list of user-defined transfer Factories
  //! We use this vector to add some special user-given factories to the
  //! Hierarchy (RAPFactory) This way the user can extend the standard
  //! functionality of the MLParameterListInterpreter beyond the capabibilities
  //! of ML.
