...
AdaptiveSaMLParameterListInterpreter<Scalar, LocalOrdinal, GlobalOrdinal,
                                     Node>::
...
AdaptiveSaMLParameterListInterpreter<
    ...
    Node>::AdaptiveSaMLParameterListInterpreter(const std::string &xmlFileName,
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void AdaptiveSaMLParameterListInterpreter<
    Scalar, LocalOrdinal, GlobalOrdinal,
    Node>::SetParameterList(const Teuchos::ParameterList &paramList_in) {
  ...
  // std::cout << std::endl << "Parameter list after CreateSublists" <<
  // std::endl; std::cout << paramListWithSubList << std::endl;
  ...
  if (verbosityLevel == 0)
    eVerbLevel = Teuchos::VERB_NONE;
  if (verbosityLevel > 0)
    eVerbLevel = Teuchos::VERB_LOW;
  if (verbosityLevel > 4)
    eVerbLevel = Teuchos::VERB_MEDIUM;
  if (verbosityLevel > 7)
    eVerbLevel = Teuchos::VERB_HIGH;
  if (verbosityLevel > 9)
    eVerbLevel = Teuchos::VERB_EXTREME;

  TEUCHOS_TEST_FOR_EXCEPTION(
      agg_type != "Uncoupled", Exceptions::RuntimeError,
      "MueLu::MLParameterListInterpreter::Setup(): parameter \"aggregation: "
      "type\": only 'Uncoupled' aggregation is supported.");
  ...
  AggFact->SetMinNodesPerAggregate(
  ...
  if (verbosityLevel > 3) { // TODO fix me: Setup is a static function: we
                            // cannot use GetOStream without an object...
    *out << "========================= Aggregate option summary "
            "========================="
         << std::endl;
    *out << "min Nodes per aggregate :               " << minPerAgg
         << std::endl;
    *out << "min # of root nbrs already aggregated : " << maxNbrAlreadySelected
         << std::endl;
  ...
  for (size_t i = 0; i < TransferFacts_.size(); i++) {
    AcFact->AddTransferFactory(
        TransferFacts_[i]); // THIS WILL BE REPLACED with a call to the
                            // MLParamterListInterpreter
  }
  ...
  if (paramList.isSublist("init smoother")) {
    ParameterList &initList =
        paramList.sublist("init smoother"); // TODO move this before for loop
    initSmootherFact = MLParameterListInterpreter::GetSmootherFactory(
        initList); // TODO: missing AFact input arg.
  } else {
    ...
    RCP<SmootherPrototype> smooProto =
        rcp(new TrilinosSmoother(ifpackType, smootherParamList, 0));
  ...
  RCP<SmootherFactory> coarseFact =
      MLParameterListInterpreter::GetSmootherFactory(coarseList);
  ...
  for (int levelID = 0; levelID < maxLevels; levelID++) {
...

    {
      ...
      // this->GetSmootherFactory(levelSmootherParam); // TODO: missing AFact
      // input arg.
      RCP<SmootherFactory> smootherFact =
          MLParameterListInterpreter::GetSmootherFactory(
              levelSmootherParam); // TODO: missing AFact input arg.
    ...
    Teuchos::rcp_dynamic_cast<TwoLevelFactoryBase>(RFact)
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<SingleLevelFactoryBase>(coarseFact)
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<SingleLevelFactoryBase>(dropFact)
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<SingleLevelFactoryBase>(AggFact)
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<TwoLevelFactoryBase>(AcFact)
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<SingleLevelFactoryBase>(nspFact)
        ->DisableMultipleCallCheck();
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void AdaptiveSaMLParameterListInterpreter<
    Scalar, LocalOrdinal, GlobalOrdinal, Node>::SetupInitHierarchy(Hierarchy &H)
    const {
  TEUCHOS_TEST_FOR_EXCEPTION(!H.GetLevel(0)->IsAvailable("A"),
  ...
  while (!isLastLevel) {
    bool r = H.Setup(levelID, InitLvlMngr(levelID - 1, lastLevelID),
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void AdaptiveSaMLParameterListInterpreter<Scalar, LocalOrdinal, GlobalOrdinal,
                                          Node>::SetupHierarchy(Hierarchy &H)
    const {
  ...
  H.Keep(
      "Aggregates",
      HierarchyManager::GetFactoryManager(0)->GetFactory("Aggregates").get());
...

  {
    ...
    Teuchos::RCP<MueLu::Level> Finest =
        H.GetLevel(0); // get finest level,MueLu::NoFactory::get()
    Teuchos::RCP<MultiVector> nspVector2 =
        Finest->Get<Teuchos::RCP<MultiVector>>("Nullspace");

    Xpetra::IO<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Write("orig_nsp.vec",
                                                                 *nspVector2);

    RCP<Matrix> Op = Finest->Get<RCP<Matrix>>("A");
    Xpetra::IO<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Write("A.mat", *Op);

    Teuchos::RCP<MultiVector> homogRhsVec = MultiVectorFactory::Build(
        nspVector2->getMap(), nspVector2->getNumVectors(), true);
    ...
    Xpetra::IO<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Write("new_nsp.vec",
                                                                 *nspVector2);

    // H.Delete("CoarseSolver",
    // init_levelManagers_[0]->GetFactory("CoarseSolver").get());
  }

  {
    ...
    for (size_t k = 0; k < HierarchyManager::getNumFactoryManagers(); k++) {
      ...
      // Teuchos::rcp_dynamic_cast<const
      // SingleLevelFactoryBase>(HierarchyManager::GetFactoryManager(k)->GetFactory("Smoother"))->DisableMultipleCallCheck();
      // // after changing to MLParamterListInterpreter functions
    }
    // not sure about this. i only need it if Smoother is defined explicitely
    // (not using default smoother) need this: otherwise RAPFactory::Build is
    // complaining on level 0
    //            and TentativePFactory::Build is complaining on level 1
    Teuchos::rcp_dynamic_cast<const TwoLevelFactoryBase>(
        HierarchyManager::GetFactoryManager(0)->GetFactory("A"))
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<const TwoLevelFactoryBase>(
        HierarchyManager::GetFactoryManager(1)->GetFactory("P"))
        ->DisableMultipleCallCheck();
    Teuchos::rcp_dynamic_cast<const TwoLevelFactoryBase>(
        HierarchyManager::GetFactoryManager(1)->GetFactory("Ptent"))
        ->DisableMultipleCallCheck();
...
void AdaptiveSaMLParameterListInterpreter<
    Scalar, LocalOrdinal, GlobalOrdinal,
    Node>::AddTransferFactory(const RCP<FactoryBase> &factory) {
...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
void AdaptiveSaMLParameterListInterpreter<Scalar, LocalOrdinal, GlobalOrdinal,
                                          Node>::SetupOperator(Operator &Op)
    const {
  try {
