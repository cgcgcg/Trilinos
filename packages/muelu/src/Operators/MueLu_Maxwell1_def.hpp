      ...
      if (precList11_.isType<double>("sa: damping factor")) {
        ...
        if (edgeDamping != 0) {
          if (!useEdgeMatrixForSmoothing) {
            GetOStream(Warnings0) << "\"sa: damping factor\" in \"maxwell1: 11list\" is set to " << std::to_string(edgeDamping) << ", but no CurlCurl matrix has been passed in \"user data\". Switching off edge-only damping. Set \"maxwell1: maxwell1: allow violation of commuting relationship\" = true to use the edge matrix." << std::endl;
